...
AI 출력 JSON에 `updateMode`와 필요 시 `filters` 필드가 포함됩니다. 프론트엔드는 이를 읽고 갱신 전략을 분기합니다:
...
- **사전 계산 대상**: 트레이더가 자주 사용하는 지표를 Hetzner 워커가 사전 계산하여 `_now` 테이블 컬럼으로 저장. 기술적 지표(MA, RSI, MACD, 볼린저밴드 등), 시장 구조(OI 변화율, 펀딩레이트 변화, 청산 누적량), 거래량 분석(거래량 변화율, VWAP), 가격 파생(ATH 대비 %, 고저가 대비 위치) 등. 사전 계산 목록은 하드코딩이 아니라 **데이터 소스 레지스트리에 등록하는 방식**이므로, 새 지표 추가 시 레지스트리 등록만으로 AI가 자동 사용 가능. 사전 계산 목록에 없는 지표를 유저가 요청 시 AI가 원시 데이터로 즉석 계산하거나 graceful하게 미지원 응답.
