...
- **해결 힌트**: Step 3+4 에서 분할 안 함(사용자 결정 2026-06-30 — 등록·마이그레이션 커밋 리뷰 집중 위해 별도 커밋 분리). 행 컴포넌트(`TableCardRow`/`TableRowDiv`) + 상태(`StatusLine`/`LoadingOrStale`)를 `TableCardRow.tsx` 형제 파일로 추출 → 본체 ~250줄. **블록킹**: No. **카테고리**: 🟡 다음(별도 커밋, Stage 1 마무리 또는 Stage 2 착수 전). **출처**: `TableCard.tsx`(현 408줄) + `M2-composable-expressiveness.md §10 Step 3+4`.
