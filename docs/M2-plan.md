...
> **★ 2026-06-09 갱신 (테마 A 분해 + Step 0·1 완료)**: 테마 A `@roadmap-milestone-manager` 6-step 분해 완료. **Step 0(F3 즉시 안전망 — coming soon allowlist) + Step 1(datasource id≠물리 테이블 분리, `[8-27]`#1 회수) ✅ 완료** (commit d343c98 + 545d29f, 6 패키지 type-check + web 139/shared 30 test). **단일 진실 = `docs/task-record/M2-themeA-card-expressiveness.md`**. **▶ /clear 후 첫 작업 = 테마 A Step 2 (IndicatorCard 단일 심볼 metric 카드).** ★ 착수 전 `[10-7]`(채널 공유 fan-out cross-talk) 먼저 회수 + crypto-domain/crypto-trader 자문. scope 정정: `[8-27]`#4/fetchKind 제외(비-거래소용), allowlist 제거는 Step 3.
>
