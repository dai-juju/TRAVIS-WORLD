...
//   콜백이 "전역 구독자 0 가드 + datasourceId 해석 + buildLiveTopics + liveBus.publish
...
//   - 토픽 문자열은 워커가 직접 조립하지 않고 buildLiveTopics 단일 진실 경유 —
//     워커/프론트가 같은 spec(buildLiveTopic 단수 = buildLiveTopics 의 마지막 원소)으로
//     조립하므로 drift 불가 (W2 grep gate 로 토픽 리터럴 0 강제).
//   - ★ buildLiveTopics(복수): optionalSelectorKeys 있는 datasource(liquidation = tape +
//     심볼별)는 누적 토픽 전부 fan-out, 없는 datasource(ticker/markPrice)는 1개 = 거동 동일.
//   - graceful: spec 없음/selector 필수 누락 시 buildLiveTopics 가 [] → 그 행 방송 skip
//     (crash 없음). 전역 구독자 0 이면 토픽 계산조차 생략 = idle 무비용.
// ============================================================

import { buildLiveTopics } from "@travis/shared";
...
export function makeTopicPublisher(
  ...
  return (marketType, rows) => {
    ...
    for (const r of rows) {
      // buildLiveTopics(복수): optionalSelectorKeys 있는 datasource(liquidation = tape +
      //   심볼별)는 누적 토픽 전부 fan-out, 없는 datasource(ticker/markPrice)는 정확히
      //   1개 = 발행 거동 동일(회귀 0). 필수 누락 시 [](graceful, crash 없음).
      const topics = buildLiveTopics(datasourceId, {
        market_type: marketType,
        symbol: r.symbol,
      });
      for (const topic of topics) liveBus.publish(topic, r);
    }
  };
}
