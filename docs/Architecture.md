...
- **Stage 4**: AI 계약 — AI 가 {datasource, form, shape, fields, transform} 독립 선택, 레지스트리 파생 검증. — ✅ **완료 (2026-07-12, 사이클 4a+4b — `M2-cycle4b-cross-screener.md §6b`)**: ① **style 축** — 카드 레벨 `style.series`(line/area/bars) override, descriptor 는 default 강등(tone/midline 도메인 가드레일은 직교 불변) ② **스코프 파생 강제** — superRefine 블록(3): `acceptsShapes ⊆ {record,series} × ds.table 존재 × queryableFields` 파생으로 marketType/symbol 필수(series 는 filters `symbol =/in` 대체 = 오버레이 보존, kline 은 table 부재 자연 면제) → self-correction 작동 ③ **크로스 metric 자유도** — 통합 `futures_indicators` 렌즈(같은 물리 테이블, 5 family 공유 const + union 등치 불변식) + **dynamicColumns**(표시 컬럼 = AI filters/sort 참조 파생 — 코드 큐레이션 0, CLAUDE.md §카드 기본값·폴백 큐레이션 금지) ④ shape 명시 필드 = **YAGNI 판정**(component+datasource 선택이 shape 를 결정 — 도입 트리거 `[10-106]`). fields 직접 선택(`data.fields`)·transform 은 `[10-106]` 원장.
