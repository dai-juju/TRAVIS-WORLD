# M2-plan §Step 2 — 실사용 피드백 수집 (📦 마감)

> **📦 마감 (2026-07-15, M3 신설 계획 세션)**: 본 문서는 이력으로 동결. **이후 실사용 발견 기록처 = `docs/M3-plan.md §6`** (백로그·hotfix 로그·관찰 체크리스트 전부 계승). 미처리 항목 처분 매핑 = `M3-plan.md §7` — ① §B O1~O15 → M3-plan §6c 이관 ② §E 미정 행 → 결정 반영(혼합 응답 보류 / `[4-28]` 트리거 유지 / 온디맨드 M4+ / 거래소 다변화 미정=베타~Launch 재논의) ③ §F 종료 게이트 → 확장 루프 병합(2026-06-08)으로 무효, 종착(우선순위 재배치)은 2026-07-15 계획 세션이 수행 ④ §H 테마 D 미착수분(F5/F6) → 기존 `[10-5]`/`[10-6]` deferred 유지. **본 문서에 처리되지 않은 잔여 없음.**
> **상태**: ~~🔄 진행 중 (2026-06-08 진입)~~ → ✅ 마감. 자유 페이스 (며칠~2주 권장 cap).
...
> **▶ 현재 (2026-07-14)**: **사이클 5(Stage 1b — BigValue/Detail 일반화) ✅ 당일 완결 = 🎉 격자 완성 선언** (`M2-cycle5-stage1b.md`) — 옛 ticker/indicator 카드 폐기 → record 14칸 개방("OI as a big number" 직교 실증). **실사용 G2 가 잠복 결함 2건 가시화 → 당일 근본 수정**: ① 차트 커서 stale rect(툴팁 우측 도달 불가 재발 — §D 계열) ② **COINM 티커 full 승격**(detail 카드가 24h 필드 영구 null 노출 = `feedback_new_card_surfaces_latent_data_defect` 5호 + `[10-14]` ticker 병합 2차 적중, §C 로그 추가). UX 관찰 5건 = `[10-111]`. **▶ 다음 = 격자 완성 후 개방 재논의(사용자 계획 세션)**: OKX / 뉴스 / ff#3 / `[10-84]` / 히트맵 / `[10-102]`(b). 실사용 병렬 계속.
> **(이력 2026-07-13 계획 세션)**: 사이클 3(`[10-35]` lag) ✅ + **사이클 4a/4b(스타일·크로스 스크리너) ✅ 당일 완결 = 🎉 Stage 4 완료** (2026-07-12). ① `[10-100]` 대청소 ✅(묘비 86건 → `deferred-archive.md`, 대장 332→219KB) ② 사이클 5 = Stage 1b 확정(roadmap-mgr 판정 "격자 완성 = Stage 1b 까지") ③ `[10-99]` UTC 표기 소사이클 ✅(`M2-[10-99]-utc-display.md`, canonical §4.4).
> **(이력 2026-07-11)**: 9회전 사이클 3 = `[10-35]` forward-fill lag 진행 — 원인 확정(per-task 예산 분할이 예산 83% stranded) + 레버 1(분할 제거) 배포(`bf203ff`, 중간 실측 5m 219→16분 / 1h 554→46분). → 2026-07-12 완결(레버 2 = `[10-103]` 강등). 단일 진실 `M2-[10-35]-forward-fill-lag.md`. + 실사용 신규 발견 2건(§D: `[10-101]` / `[10-102]` — 사이클 4 로 당일 회수) + `[10-100]` 별도 세션 확정.
> **(이력 2026-07-10)**: 확장 루프 **8회전 사이클 2 (GenericChart) ✅ 완결** — Step 6 라이브 G2 7종 전부 PASS(site=DB Binance 공식 API 3연속 8자리 일치 + AI 분기 log_chat 8/8 + ★COINM 명시 쿼리가 Custom Instructions soft default 를 이김 = 테마 C 검증 동반) + **G2 적발 결함 3건 당일 수정**(N1 last_settled=predicted 네이밍 트랩[M1.8 잠복 위생 #9, `c2515ae`] / RF 줌 툴팁 오스냅 / y축 부호 소실[`ec4bb06`, `[10-92]` 회수]). 배포까지 당일 완료(worker→collector 재시작 + 첫 사이클 USDM 702/COINM 20 심볼 반영 DB 정합 실측). **다음 사이클 = `[10-35]` forward-fill lag (사용자 확정 2026-07-10 — 차트가 가시화한 사용자-facing 신선도 결함, Stage 4 선행)**. 단일 진실 `M2-cycle2-genericchart.md §4i`. 실사용 병렬 계속(`[10-83]` advisory 관찰).
> **(이력 2026-07-09 #2)**: 확장 루프 **7회전 사이클 1 ✅ 완결(G3 PASS = `[10-77]` 묘비)** + **8회전 사이클 2(GenericChart) 🔄 Step 1~6 코드+데이터 ✅ — 잔여 = Step 6 라이브 G2 만**. Step 5 라이브 G2 전 게이트 PASS(AI 자율 분기 5/5 + ★"compare" 직교 실증 3호 + hotfix 4연쇄) + UIUX 4결정 + **Step 6 펀딩(§4h)**: 별도 이벤트 테이블 `history_futures_funding`(도메인 자문이 "기존 컬럼 채움" 기각 — interval 버킷 복제=가짜 포인트) + funding rate-limit 버킷(Plan 검증 결함 적발) + **backfill 완주**(USDM 196,600행/60일 + COINM 3,580행, 라이브 smoke 가 페이지 절단 유실 적발→수정) + chart-card bars/stepped 어휘(펀딩 전용 카드 아님). reviewer 2회 0C. 신규 `[10-94]`~`[10-96]`. 단일 진실 `M2-cycle2-genericchart.md §4h`. **다음 세션 = Step 6 별도 라이브 G2(§4h 끝 게이트 7종) → 통과 시 사이클 2 완결**. 실사용 병렬 계속(`[10-83]` advisory 관찰).
> **(이력 2026-07-06)**: 확장 루프 **6회전 = "Feed form + 청산" ✅ 완결** (= Composable Stage 3 events 첫 시민 = ff#2 재개·완성). feed-card(events 2번째 form) + 청산×table-card(같은 데이터 두 형태 AI 자율 분기 라이브 실측) + 피드 과거 seed + CM migration 오염 당일 해소. 단일 진실 `M2-pathA-ff2-liquidation.md`.
> **(이력 2026-06-28) — 🎯 방향 전환**: ff#2 Step 5 착수 직전, 사용자 지적("컴포넌트가 데이터 종류로 하드코딩 = 내 방향 아님")으로 **Form↔Data 직교(모든 데이터 × 모든 형태)** 를 **모든 M2+ 개발의 중심축**으로 확정. **ff#2 일시 정지** → 다음 = **테마 "Composable Expressiveness"**(아래 §H "M2 테마 — Composable Expressiveness"). 중심축 단일 진실 = `CLAUDE.md §최상위 개발 축` + `PRD.md §2 모든 데이터 × 모든 형태` + `Architecture.md §8 Form↔Data 직교` + 메모리 `project_composable_expressiveness_axis`. ff#2 완료분(Step 1~4: 토픽 keystone·liquidation 플립·워커 publish·`useDataServiceFeed` 훅)은 events shape 토대로 **전부 재사용**(청산=Stage 3 events 첫 시민). **실 step 분해는 다음 세션 `@roadmap-milestone-manager`.**
...
| 2026-07-14 | COINM 티커 24h change/VWAP/체결수 전 심볼 "—"(사이트는 표시) + quote_volume 에 base 수량 오적재 (detail 카드가 가시화) | **COINM 만 miniTicker 잔존**(full 승격 누락 — mini 부재 필드를 매초 null 로 기입해 REST 보강 클로버링) + WS normalize 가 marketType 무관 q→quote_volume (COINM q=base 수량) | Stage 1b G2 hotfix (`3454239`) / `M2-cycle5-stage1b.md §8b`. 검증: DB 3심볼 변화율 채워짐 + base_volume 산식 정합(6,787.5 BTC) + st 가드 UM 혼입 0 (★스모크가 dstream/fstream ticker UM+CM 병합 실측 = `[10-14]` 2차 적중) |
...
| 2026-07-11 | **차트 스타일 자유도 욕구** — "펀딩비를 단순 선차트로" 같은 요구가 불가 (seriesStyle 이 descriptor 고정, AI 계약에 style 부재). 사용자 원칙: 말도 안 되는 조합 아니면 자유롭게 | D | `[10-101]` | 모든 데이터 × 모든 형태의 **스타일 하위축** — Stage 4 동반 후보 |
| 2026-07-11 | **크로스 복합 쿼리 욕구** — "Low LS ratio and top gainers" 가 2카드 분리/한 조건만 반영. 다른 datasource 조건 AND 스크리닝 불가 | D | `[10-102]` | (a) 같은 테이블 = queryableFields 개방(싼 회수) / (b) 크로스 테이블 = future.md §5 1단계(derived datasource) 활성화 사안 — 테마 후보 |
...
### 🎯 M2 테마 — Composable Expressiveness (Form↔Data 직교) — Stage 1 ✅ + Stage 3 events ✅
...
> **진행 (2026-07-12)**: 사이클 2 GenericChart ✅ + 사이클 3 `[10-35]` lag ✅ + **사이클 4 (4a 스타일 + 4b 크로스 스크리너) ✅ 전부 당일 완결 = 🎉 Stage 4 완료**("simple line chart" 실증 / "low LSR × rising OI" 크로스 필터 실증 / 스코프 스키마 강제 — PRD §2 "쿼리 자유도" 명문화 동반, 사용자 재강조 2건[스타일·큐레이션 금지] CLAUDE.md 반영). 단일 진실 = composable §11 + `M2-cycle4a/4b` task-record.
