...
> **상태**: ✅ **fast-follow #1 (마크가격/펀딩) 완결 (2026-06-26)** — Phase A(1·2·3 휴면) + Phase B(4 워커재배포 / 5 플립 / 6 라이브 G2) 전부 완료. **라이브 G2 5게이트 통과**(박동소멸+혼합무손실+site=DB+ES256+`[10-62]` 해소). ★ 라이브 사고: marketType 누락→frozen→2겹 hotfix `54d7b98`. 본 §4 가 단일 진실.
> **▶▶ `/clear` 후 다음 세션 순서 (사용자 결정 2026-06-26)**: ① **워커 W1 경고 정리** — Hetzner `178.105.38.94` `git pull`(현 `8575f7c`→최신, transport:ws_direct 정합으로 W1 과도기 경고 소멸) + restart(★SIGKILL `[10-31]` 정상). ② **basis 418 모니터링** `[10-69]`(우리 IP·경로 A 무관, basis freshness 관측). ③ **`@crypto-trader` 라이브 펀딩 카드 평가**(옵션 C 재연결 UX + 펀딩 카드 실사용성, advisory). → ④ **fast-follow #2 (청산 forceOrder 카드)** 착수, **`[10-68]`(publish 배선 헬퍼 추출) 선결**. ⑤ (병행 탐색 후보) **`[10-70]` WS-derived metrics 컴퓨팅 레이어** — 사용자 제안(basis 등 WS 1차값 내부 실시간 계산), crypto-domain 검증 후 별도 테마 판단.
