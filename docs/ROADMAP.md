...
- **▶ 사이클 4 — Stage 4 AI 계약 + 쿼리 자유도 묶음 (사용자 확정 2026-07-12)**: Stage 4(form·data·shape·fields 독립 선택 계약) + `[10-101]` 차트 seriesStyle 자유("펀딩비를 선차트로") + `[10-102]`(a) 같은 테이블 크로스 필터 개방("Low LSR × top OI") + `[10-91]`/`[10-78]` 스키마 파생 강제. 배경 = **PRD §2 "쿼리 자유도" 명문화(2026-07-12 사용자 재확정 — 모든 데이터 × 모든 form × 스타일, "유저의 어떤 요청이든")**. 착수 시 plan mode + `@roadmap-milestone-manager` 분해.
