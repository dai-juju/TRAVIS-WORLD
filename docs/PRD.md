...
- **정의 정밀화 (2026-07-21 사용자 확인)**: 목표는 "모든 데이터 × 4모양 강제"가 아니라 "**의미 있는 (데이터 × 모양) 조합은 하나도 막혀 있으면 안 된다**"이다. 저장은 항상 1벌 — shape 는 저장 형식이 아니라 **서빙 시점의 계약**이며, 같은 물리 데이터가 여러 shape 로 서빙된다(청산 1테이블 → 피드·표·차트). **막힌 칸(구조가 못 여는 조합) 0 = 격자 완성의 실체**이고, 빈 칸(원료 부재·수요 대기)은 reshape 배관 1회로 열린다. 실측 커버리지 매트릭스(living) = `docs/Architecture.md §8 커버리지 매트릭스`.
...
- **🏁 M2 종료 → M3 "Binance 우주 완성" (2026-07-15 신설, 단일 진실 `docs/M3-plan.md` + `ROADMAP.md §M3`)**: 신규 데이터 **공급자** 추가 전, Binance 데이터만으로 모든 데이터 × 모든 형태 × **모든 인터랙션**(§4 spawn/drill-down 실동작 wire 포함) + 고도화 + UIUX 완성. 경계 = 공급자 기준(Binance 내부 datasource 추가는 IN — 알파/체결·호가 등). 첫 사이클 = 인터랙션 wire — **✅ M3-step1 완결 (2026-07-16)**: Spawn 실동작(§4 각주 참조) + 라이브 G2 PASS + AI 자율 actions 선언 실증 (`task-record/M3-step1-interaction-wire.md`). **✅ M3-step2 완결 (2026-07-17)**: 인터랙션 완성 2탄 — 뷰포트 빈자리 배치 + 재클릭 체인 깊이 1 + hover 힌트 + 엔진 스코프 보충(§4 각주 참조, `task-record/M3-step2-interaction-2.md`). **✅ M3-step3a 완결 (2026-07-19)**: `[10-84]` 청산 events→series 시간버킷 집계 — 같은 청산 데이터가 이제 **피드(events)·표(set)·차트(series)** 세 형태 전부로 표현된다(§2 "모든 데이터 × 모든 형태" 직교 실증 3호, chart form 코드 0줄 유입). 집계는 DB 함수가 수행(클라 집계는 3,000행 상한에 무증상 절단되는 사이트=DB 위반). 심볼 생략 = 전 시장 집계 / `side` 필터로 롱·숏 분리. 버킷·기간·형태·범위는 **전부 AI 가 유저 쿼리에서 파생**(코드 큐레이션 0). `[10-81]`(AI 현재시각 주입) 동반 회수 → "지난 1시간" 류 상대시간 쿼리 가능. 단일 진실 `task-record/M3-step3a-liquidation-series.md`. **✅ M3-step3b 완결 (2026-07-21)**: Phase 2 — 청산 **롱(위)·숏(아래) 다이버징 한 차트**(y축 공유 = 롱:숏 비대칭이 실제 비율로 보임)가 기본이 되고, 합계/롱만/숏만은 AI 가 `style.breakdown`·`side` 필터로 유저 문장에서 파생(도메인 기본은 AI 명시값에 항상 양보). AI 시간창 필터("12:00~20:00 UTC")도 fetch 에 실반영(history 6종 포함). 단일 진실 `task-record/M3-step3b-chart-multicolumn.md`. **✅ M3-step4 완결 (2026-07-22)**: 린 워커 위생 — 워커 재시작이 "7분 수동 좀비"에서 **11초 자동 완주**가 되고(7/14 실사고 근본 해소), 청산 마켓 오염이 거래소 권위 판별자(`o.st`)+fail-closed 로 구조 차단되며, **신규 상장 코인의 모든 데이터(가격·지표·청산·실시간)가 상장 후 ≤1시간 내 자동 반영**된다(1h 명단 갱신 + 새 코인 감지 시 워커 자기 재시작 — §7 사이트=DB 원칙의 "신규 상장" 커버리지 완성). 단일 진실 `task-record/M3-step4-lean-worker-hygiene.md`. 이후 스케치: M4 외부 데이터소스(CMC·뉴스) → M5 베타 준비(M1.7 활성화 + 100명 스케일 + 보안·정리) → 베타테스터 모집 → Launch §L. (세션 컨텍스트·혼합 응답 = 보류 / OKX = 베타~Launch 사이 재논의.)
