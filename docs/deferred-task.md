...
### [10-57] 프로덕션 워커(178.105.38.94) 커널 재부팅 — 보안 패치 적용 (저트래픽 시간대)
- **근본 (security-auditor 노출-직후 재감사 W-4, 2026-06-22)**: Ubuntu 24.04 가 kernel `6.8.0-107`→`124` 업데이트 후 **재부팅 대기**(`apt install` 중 "Pending kernel upgrade" 반복 경고). 패치된 커널 취약점 미적용 창. 박스가 이제 443 인터넷 노출 + DNS-only origin IP 공개라 패치 지연이 의미를 가짐(단 외부 포트 22/80/443 으로 좁아 즉각 위협 제한적).
- **해결 힌트**: 가까운 저트래픽 시간대 `reboot`. **재부팅 후 필수 재확인 4종**: ① `journalctl -u travis-worker | grep "JWT 인증 활성"` (fail-closed 자동 secret 로드) ② `ss -tlnp | grep 8081` = `127.0.0.1:8081`(0.0.0.0 회귀 아님) ③ Caddy/ufw/travis-worker 자동 기동(전부 `enabled`) ④ 수집(경로 B) 재개 + WS 인증서 유효. **블록킹**: No. **카테고리**: 🔵 Launch Readiness (운영).
