...
6. **▶ (다음) 사이클 4 = Stage 4 AI 계약 + 쿼리 자유도 묶음 (사용자 확정 2026-07-12)**: Stage 4(form·data·shape·fields·transform 독립 선택 — `[10-91]`/`[10-78]` 스키마 파생 강제 동반) + `[10-101]` seriesStyle 자유 + `[10-102]`(a) 같은 테이블 크로스 필터 개방. 배경 = **PRD §2 "쿼리 자유도"(2026-07-12 사용자 재확정, 영구): 모든 데이터 × 모든 form × 스타일 + "유저의 어떤 요청이든"**. 이후 Stage 1b(BigValue/Detail — `[10-74]` 자연 흡수) · 히트맵/게이지 등 composable 계속 · `[10-102]`(b) 크로스 테이블(future.md §5). **타 거래소(OKX 등)·뉴스·ff#3(체결/호가)은 격자 완성 후** (사용자 확정 2026-07-07, 큰 그림 정렬 역질문 답변).
