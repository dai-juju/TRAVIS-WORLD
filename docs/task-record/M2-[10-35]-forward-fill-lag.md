...
> **상태**: ✅ **완결 (2026-07-12) — 레버 1 만으로 Step 3 신선도 게이트 "충분" 판정(사용자). 레버 2(GROUPS 재편) = `[10-103]` deferred 강등. §4d after 실측 / §4e 마감.**
...
## 4d. Step 2 확정 + Step 3 게이트 ✅ — 정상상태 after 실측 (2026-07-12, 배포 +23h, 2회 샘플)

**after 표 (05:11 / 05:14 UTC 2회 샘플 — §3 과 동일 freshness 인덱스 LATERAL, USDM 9 interval + COINM)**:

| interval | baseline (07-11) | after USDM (2회) | after COINM | 판정 |
|---|---|---|---|---|
| **5m** | 🔴 219분 (최악 8.6h) | **91~93분** | 16→**3분** (샘플 사이 순회 통과 포착) | ✅ 최악 ≈ 단주기 순회 1바퀴 ~1.5h |
| 15m | 44분 | 56~58분 | 26~28분 | ✅ |
| 30m | 🔴 404분 | **11~13분** | 11~13분 | ✅ |
| **1h** | 🔴 554분 (9.2h) | **71~73분** | 71~73분 | ✅ |
| 2h~6h | ~374분 | 311~313분 | 71~313분 | 봉폭(2~6h) 감안 무난 — mid 순회 ~5h |
| 12h / 1d | 1094/1814분 (COINM) | 봉 마감 후 ~5h (1031/1751분) | 동일 | 봉폭(12/24h) 대비 무난 |

**안전 게이트 (3h 창 재확인)**: 429 = 52건(~17/h — baseline ~13/h 질감 복귀, catch-up 일시 ~40/h 에서 하강) / **비-basis -1003 = 0건**(진짜 quota 위반 없음) / basis -1003 = 기존 Binance LB 노이즈(§4b 판정 유지). G2 ③ 차트 freshness 는 DB 실측이 카드 "last point (N ago)" 라벨의 원천이므로 위 표로 갈음(실사용 중 상시 노출 — 별도 세리머니 생략).

**★ Step 3 게이트 = 사용자 판정 (2026-07-12): "충분" — 레버 2(GROUPS hot/warm/cool/cold 재편) 생략, `[10-103]` deferred 강등.** 근거: 단주기 최악 8.6h→~1.5h(~6배), 물리 하한 28분 대비 레버 2 잔여 이득(~1.5h→~30분)이 코드 복잡도·장주기 신선도 희생 대비 낮음. 실사용 체감으로 부족 판명 시 `[10-103]` 재승격.

## 4e. Step 5 마감 ✅ (2026-07-12)

- `[10-35]` 묘비 (deferred-task.md) + **레버 2 → `[10-103]` 신설**(⚪ 무기한, 실측 기반 restMs 튜닝 힌트 보존) + **`[8-31]`ⓓ 재평가 = 별개 복원력 축으로 존치** — circuit breaker 는 basis -1003 을 못 막고([8-31] 2026-06-06 판정: Binance 측 원인) 레버 1 과도 무관, 일반 복원력 항목(📋)으로만 정당 → 본 사이클에서 회수하지 않음.
- 상위 문서 전파: ROADMAP §다음 2-사이클 / usage-feedback §H / composable §11 / MEMORY.
- **동일 세션 부수 (2026-07-12)**: 사용자 "쿼리 자유도" 원칙 재확정 → `PRD §2` + `CLAUDE.md §최상위 개발 축 4` + `[10-101]`/`[10-102]` 재강조 명문화 (모든 데이터 × 모든 form × 스타일, "유저의 어떤 요청이든").
- **▶ 다음 사이클(사이클 4) = Stage 4 AI 계약 + 쿼리 자유도 묶음 (사용자 확정 2026-07-12)**: Stage 4(form·data·shape·fields 독립 선택 계약) + `[10-101]`(seriesStyle 자유) + `[10-102]`(a)(같은 테이블 크로스 필터 개방) + `[10-91]`/`[10-78]`(스키마 파생 강제) — 착수 시 plan mode + `@roadmap-milestone-manager` 분해 + `@zod-schema-architect`/`@ai-orchestrator-specialist` 자문.
...
| 2026-07-12 | Step 2~3 | ✅ 정상상태 after 표(2회 샘플, §4d) + 안전 게이트(비-basis -1003 = 0 / 429 ~17/h) → **사용자 게이트 "충분" = 레버 2 생략**(Step 4 스킵, `[10-103]` 강등). |
| 2026-07-12 | Step 5 | ✅ 마감(§4e) — `[10-35]` 묘비 + `[10-103]` 신설 + `[8-31]`ⓓ 존치 판정 + 상위 문서 전파 + commit/push. **사이클 3 완결.** 다음 = 사이클 4(Stage 4 + 쿼리 자유도 묶음). |
