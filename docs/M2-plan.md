...
> **현재 위치 (2026-06-06)**: **M1.9 ✅ 완료** — Step 0~3 전부 통과, 종단 게이트 G1~G5 ✅. forward-fill USDM(23h+ 무중단 `NRestarts=0`) + COINM(06-06 롤아웃, `markets=[usdm,coinm] tasks=6`) 라이브 가동 + site=DB 소수점 일치(USDM ~50셀 + COINM 24셀, OI contract 단위 검증) + ⓑ AbortSignal graceful 종료 2회 검증 + basis -1003 메커니즘 규명(Binance LB 노드 weight 풀 혼잡, basis weight 0, 우리 무관 — backoff 흡수). 회수: `[8-26]`/`[8-3]`/`[8-20]`/`[8-31]`ⓐⓑⓒ/`[8-33]`/`[3-68]`. 잔여(차단 아님): COINM 24~48h 모니터링 / `[8-31]`ⓓ circuit breaker / `[8-34]` COINM LSR sanity guard. **다음 = 본 §Step 2 (본인 실사용 + 베타테스터 피드백 수집).** 단일 진실: `docs/task-record/M1.9-complete.md`.
