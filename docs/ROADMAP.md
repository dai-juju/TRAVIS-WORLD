...
| **8.2a** | worker 신규 fetcher + fundingInfoTask (24h) + premiumIndexTask (30분) + perSymbolTask 통합 + unit test. **8.2a-1 ✅ 2026-05-26** (worker 컬럼명 hotfix + Hetzner deploy, `docs/task-record/M1.8-step1-hotfix-rename-funding.md`). **8.2a-2 A+B ✅ 2026-05-26** (`fetchBasisBatch` / `fetchFundingInfo` / `normalizeUsdmBasis` + `fundingInfoTask` + `premiumIndexTask` + IDataService `updateSymbolFundingIntervalHours`, `docs/task-record/M1.8-step2a-2-fetchers.md`). **8.2a-2 C~G 🟡 진행 예정** (perSymbolTask 통합 + worker bootstrap + rate-limited dispatcher + unit test + Hetzner deploy). | `[8-2]` annualizedBasisRate (보류 확정) | 각 fetcher 1~2 case test PASS + TRADING allowlist 사전 필터 적용 | 2~3h (8.2a-2 C~G ~1.5h 잔여) |
