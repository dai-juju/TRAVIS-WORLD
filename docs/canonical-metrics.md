...
- **Liquidation feed (`!forceOrder@arr` WS)** — 🔄 ff#2 진행 중 (단일 진실 `task-record/M2-pathA-ff2-liquidation.md`). **Canonical 정의 (crypto-domain-expert 자문 확정)**:
  ...
  - **★ CM migration (2026-06-30 발효, ⚡[10-14] 적중)**: Binance UM/CM 아키텍처 통합으로 `!forceOrder@arr` 가 fstream·dstream 양쪽에서 **UM+CM 병합** push + `st`(1=UM/2=CM)·`ps` 필드 도입. ref: All-Market Liquidation Order Streams + change-log 2026-06-10 (조회 2026-07-06). 다른 dstream @arr 스트림 확대 여부는 `[10-14]` 상시 감시.
    - **🔴🔄 2026-07-22 재정정 (M3-step4 라이브 캡처) — `st` 는 "부재"가 아니라 페이로드 최상위가 아닌 `o` 객체 안에 실린다.** dstream 라이브 8/8 프레임 실측: `{"e","E","o":{...,"ps":"...","st":1}}` — 최상위 키는 `e/E/o` 뿐. **공식 문서 예시는 최상위에 표기 = 문서↔와이어 괴리.** 구 워커 코드(2026-07-06)는 최상위(`raw.st`)에서 읽어 st 분기가 통째로 죽은 코드가 됐고, 2026-07-19 의 "st 부재" 정정은 이 **중첩 위치 오독**의 하위 결론이었다 (역산 증명 자체는 유효 — "최상위 st 는 undefined"). 교훈 확장: 필드의 **존재**뿐 아니라 **중첩 위치**도 스트림·문서를 신뢰하지 말고 라이브 프레임으로 확정할 것 (`feedback_external_api_live_smoke`).
    - 유일 방어선이던 멤버십 폴백이 **fail-open**("상대 마켓 allowlist 에 **있다고 확인될 때만** drop)이라, 워커 인메모리 allowlist(24h 갱신)에 없는 **신규 상장 심볼이 그대로 통과**했다. ticker/markPrice 는 "자기 마켓 allowlist 에 있는 것만 수용"(fail-closed)이라 구조적 면역. **✅ 2026-07-22 (M3-step4) 워커 근본 수정**: 2단 가드 — ① `o.st` 권위 판별(상장 0초부터 정확) ② st 부재 폴백 = fail-closed inclusion("경로 B 이력 보존" 정책 폐지 — 청산은 TRADING 심볼에서만 발생하므로 실제 이력 손실 없음) + refresh 24h→1h + 신규 상장 자기 재시작. 상세 = `task-record/M3-step4-lean-worker-hygiene.md` (`[10-118]` 근본해결(동적 재구독)만 별건 잔존).
