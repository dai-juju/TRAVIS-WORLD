...
> **(이력) 코드 + DB 단계**: commit `e0f34e2`~`58f9f8e` 5개 push (2026-06-12). Step 1~5 전부 완료 — 마이그레이션 Dashboard 적용 + backfill NULL 0/2,160 + 구워커 비파괴 실측 + regen 일치 검증 + 자문 2종 **Critical 0**. (이후 남은 게이트 3 은 위 ✅ 완결 줄과 §5 대로 같은 날 전부 통과.)
> **단일 진실**: 본 파일 = 테마 B 전체 추적처. 발견 맥락 = `M2-step2-usage-feedback.md §H` (F2). deferred = `[10-2]`(묘비) + 설계 단계 신규 `[10-24]`~`[10-29]` + 게이트 단계 신규 `[10-30]`~`[10-35]` (§7).
