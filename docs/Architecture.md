...
**symbols 마스터 동기화 (`[10-22]` 회수, 2026-06-11)**: `syncSymbolsTask` 가 3마켓 exchangeInfo 를 **부팅 시 1회 + 24h 주기**로 `symbols` 테이블에 upsert (데이터 위생 #3 이행). 부팅 순서는 sync → loadAllSymbols — 신규 상장 심볼이 allowlist/WS 구독 스냅샷에 즉시 반영된다. 신규 심볼의 WS 실시간 합류는 워커 재시작 시점(부팅 스냅샷 정책) — 그 사이는 REST 폴링이 채움. 동적 WS 구독·즉시 감지 옵션은 `deferred [10-23]`.
