...
- **사이클 3 — `[10-35]` forward-fill lag 해소** 🔄 **진행 중 (2026-07-11 착수, 단일 진실 = `task-record/M2-[10-35]-forward-fill-lag.md`)**: 원인 확정 = `perTaskReqPerMin 150÷6=25/min` 정적 분할이 steady-state 예산 ~83% stranded(전역 token-bucket `[8-31]`ⓐ 도입 이전 잔재 — cycle 하한 8.4h ≈ 관측 8.6h 정합). **레버 1(분할 제거) 구현·배포 완료**(`bf203ff`, reviewer 0C/0W, 회귀 핀 4 test) — 중간 실측 5m lag **219→16분** / 1h **554→46분**, 안전 게이트 통과(★비-basis -1003 = 변경 전후 모두 0 / basis -1003 = 기존 Binance LB 노이즈, baseline 4,149건/24h 대비 감소). **잔여 = 정상상태 after 표(배포+1.5h~, 신선도 목표 "실측 보고 결정") → Step 3 레버 2(GROUPS hot/warm/cool/cold 티어 재편) 결정 게이트(사용자) → (조건부 Step 4) → Step 5 docs·`[10-35]` 묘비·`[8-31]`ⓓ 재평가.**
- **이후**: Stage 4 AI 계약(`[10-91]`/`[10-78]` 스키마 파생 강제 + `[10-101]` 차트 스타일 자유도 동반 후보) → Stage 1b(BigValue/Detail) → `[10-102]` 크로스 복합 쿼리(실사용 욕구 2026-07-11, future.md §5 1단계 — 테마 후보). **타 거래소·뉴스·ff#3(체결/호가)은 격자 완성 후**.
