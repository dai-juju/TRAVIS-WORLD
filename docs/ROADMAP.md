...
| **8.2a** | worker 신규 fetcher + fundingInfoTask (24h) + premiumIndexTask (30분) + perSymbolTask 통합 + unit test + rate-limit monitoring. **8.2a-1 ✅ 2026-05-26** (worker 컬럼명 hotfix + Hetzner deploy, `docs/task-record/M1.8-step1-hotfix-rename-funding.md`). **8.2a-2 A~F ✅ 2026-05-26** (A: types/normalize/adapter / B: fundingInfoTask + premiumIndexTask + IDataService / C: perSymbolTask USDM 6 fetcher 직선 + worker bootstrap / D: client.ts rate-limit module state + 90% sticky throttle 2배 60초 / E: normalize.test.ts 6 case / F: type-check + lint + test 63 PASS — `docs/task-record/M1.8-step2a-2-fetchers.md`). **8.2a-2 G 🟡 진행 중** (commit `feat(m1.8-step2a-2-CDEF)` + Hetzner deploy + Supabase 검증). | `[8-2]` annualizedBasisRate (보류 확정), `[8-10]` full rate-limit dispatcher (M2+ 등재) | type-check + lint + test 63 PASS ✅ + TRADING allowlist 사전 필터 적용 (commit 직전) | 2~3h (8.2a-2 G ~30~45m 잔여) |
