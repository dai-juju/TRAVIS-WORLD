...
> **상태**: ✅ **Stage 1 완료 (Step 1~5, 2026-06-30)** — `table-card` 가 옛 카드 2종(`coin-list-card`/`indicator-list-card`)을 **라이브 대체**. 게이트 registry 권한 일원화 · alias 대신 저장뷰 삭제 · all-view 일관화(`defaultLimit` 필드 제거). **라이브 G2 PASS**: top gainers/all spot(449 가상스크롤)/**top OI=옛 F3 "realtime error" 해소(site=DB 자릿수 일치)**/funding + 단일/다중 의도분리(zod S2 해소) + COINM 단위 + 콘솔 0. 커밋 `2a964bd`(Step 3+4)·`f1221ae`(docs)·all-view 수정(이 세션 커밋). **사이클 1(`[10-77]`+`[10-82]`) ✅ 완결 (2026-07-09 G3 PASS 묘비). ▶ 사이클 2 GenericChart 🔄 Step 1~5 + UIUX ✅ (2026-07-08~09) — 남은 것 = Step 6(펀딩)만**: Shape 계약(2층 게이트) + `useDataServiceSeries` + history 6종·chartDescriptors + uPlot form + **Step 5 등록·원자적 일괄 push·라이브 G2 전 게이트 PASS**(AI 자율 분기 5/5 + ★"compare" 시간축 단서로 table↔chart 갈림=직교 실증 3호 + site=DB 모양 일치 + **라이브 hotfix 4연쇄**[marketType 500/카드 축소/RO 소진/★uPlot.min.css=DPR 잘림]) + **UIUX 확장**(사용자 결정 4건: 플로팅 툴팁/토글 9종 registry 파생/포인트수 유지/freshness). **단일 진실 = `M2-cycle2-genericchart.md`** (§4g 끝 = Step 6 착수 가이드). **다음 세션 = Step 6 (funding_history: collector 7번째 task + backfill + 이산성 canonical + 별도 G2).**
