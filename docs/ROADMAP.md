...
- **사이클 2 — GenericChart (Composable Stage 2+3)** 🔄 **진행 중 — Step 1~4 ✅ (2026-07-08 하루 완주, 단일 진실 = `task-record/M2-cycle2-genericchart.md`)**: ① Shape 계약(servableShapes×acceptsShapes **2층 게이트** — 렌더 게이트=dataShapes 유지 byte-identical, shape=등록/테스트 불변식 레이어) ② `useDataServiceSeries`(4번째 훅, 첫 주기 pull — per-symbol 병렬 7ms vs `IN` 500ms EXPLAIN 실측) ③ history datasource 6종(`*_history`) + chartDescriptors 팩(OI 모노크롬·1.0/0 midline·null=gap) ④ GenericChart form(**uPlot 1.6.32** — lightweight-charts 기각, 미등록 격리). **선결 실측: 펀딩 히스토리 0행 확정 → Step 6 = collector 7번째 task + backfill.** ⚠️ **Step 3·4 = 로컬 커밋만(`09cb300`·`d16863c`) — push 는 Step 5(chart-card 등록)와 원자적 일괄**(form 없는 datasource 의 AI 노출 = 쿼리 fallback 퇴행 창 방지, 사용자 결정). **▶ 남은 것 = Step 5(등록+일괄 push+라이브 G2) → Step 6(펀딩+별도 G2)**. 오버레이 = (a) 다중 심볼만((b) 다중 metric 이중축 = Stage 4 펜싱 / (c) 가격 = `[10-88]` 데이터 갭). 청산 집계 배관 = `[10-84]` 후속.
