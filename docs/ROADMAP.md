...
- **사이클 4 — Stage 4 AI 계약 + 쿼리 자유도 묶음 (사용자 확정 2026-07-12, 테마급 실측 13~19h → 4a/4b 분리)**: 배경 = **PRD §2 "쿼리 자유도" 명문화(2026-07-12 사용자 재확정 — 모든 데이터 × 모든 form × 스타일, "유저의 어떤 요청이든")**.
  ...
  - **▶ 4b — `[10-102]`(a) 크로스 metric 스크리너 + `[10-91]`/`[10-78]` 스키마 파생 강제 (후속 세션)**: 통합 `futures_indicators` datasource(공유 const + 등치 불변식 — 확장성 사용자 조건) + dynamicColumns(필터된 값 표시) + superRefine 단일 대상 스코프(acceptsShapes 파생, false-positive 0 게이트 + [10-79] 가상화 헤더 게이트). 계획 개요 승인됨(plan `serialized-wibbling-pebble.md`) — 착수 시 상세 재확정.
