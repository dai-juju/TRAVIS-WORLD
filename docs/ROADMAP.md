...
>
> **★ Step 3+4 ✅ 완료 (2026-06-30, 미커밋)** — `table-card` 양쪽 등록 + 게이트 일원화(registry 권한 단일 진실) + 옛 카드 2종 폐기. ★ alias 대신 **저장뷰 전체 삭제**(사용자 결정, 베타 전). type-check 6패키지 clean + shared 61/web 334 회귀 0 + ESLint exit 0 + 자문 4종(nextjs/code-reviewer/zod/security) **0 Critical**. zod W1(requiredFields 무결성 불변식)·code-reviewer W1/W2(stale 주석) 반영, W3(파일 분할)=`[10-76]` 연기. 다음 = Step 5 라이브 G2 → **확정 순서(사용자 2026-06-30): ② Feed form+청산(=ff#2 재개=Stage 3 events 시민, 옛 LiquidationFeedCard→모양-제네릭 Feed form=안티패턴 회피, ff#2 자산 재사용) → ③ Realtime throttle `[10-77]` → ④ GenericChart/Stage 4**. 단일 진실 `M2-composable-expressiveness.md §10·§11`.
>
