...
4. **향후 경험도 이 축 위에 있다**: 형태 사후 변경("이걸 차트로 바꿔줘") · 카드끼리 연동(한 카드 클릭→연관 카드) · `reactive` 갱신 모드 · 표현 스타일 자유("펀딩비를 선차트로", `[10-101]`) · 복합/크로스 쿼리("Low LSR and top gainers", `[10-102]`).
   - **🔒 쿼리 자유도 (2026-07-12 사용자 재확정, 영구 방향)**: 이 축은 선물 지표 등 특정 데이터군에 국한되지 않는다. **모든 데이터가 모든 컴포넌트로** 표현될 수 있어야 하고(모양 비호환 등 "너무 아닌 것"만 예외), TRAVIS 는 **유저의 어떤 요청이든** 원하는 데이터를 원하는 형태로 보여주는 것을 목표로 한다(PRD §2). 새 컴포넌트·새 데이터 추가 시 이 방향을 절대 잊지 말 것.
...
모든 연기/보류/미뤄진 결정은 `docs/deferred-task.md` 에 **즉시 기록**, 처리 완료 시 **즉시 제거 + 전문을 `docs/deferred-archive.md` 로 이관** (인라인 묘비 금지 — 2026-07-13 `[10-100]` 대청소로 확립).
