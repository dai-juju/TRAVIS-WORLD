...
> **▶ 현재 (2026-07-09)**: 확장 루프 **7회전 사이클 1 ✅ 완결(G3 PASS = `[10-77]` 묘비)** + **8회전 사이클 2(GenericChart) 🔄 Step 1~5 + UIUX ✅ — 남은 것 = Step 6(펀딩)만**. Step 5 라이브 G2 전 게이트 PASS(**AI 자율 분기 5/5** + ★"compare" 시간축 단서 유무로 table↔chart 갈림 = 직교 실증 3호 + site=DB 모양 일치) + **라이브 hotfix 4연쇄**(marketType 500/카드 축소/RO 소진/★uPlot.min.css 누락=DPR 1.25 잘림 — 전부 사용자 실환경에서만 가시화 = 라이브 G2 존재 이유 실증) + **UIUX 확장 사용자 결정 4건**(플로팅 툴팁/토글 9종/포인트수 유지/freshness "last point"). 부수: `[10-35]` forward-fill lag 사용자-facing 실증(재평가 후보). 단일 진실 `M2-cycle2-genericchart.md`. **다음 = Step 6(펀딩 적재+별도 G2)**. 실사용 병렬 계속(`[10-83]` advisory 관찰).
