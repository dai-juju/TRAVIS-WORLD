...
- **Stage 1**: 스냅샷 form 통합 (`coin-list` + `indicator-list` → 모양-제네릭 Table; BigValue/Detail 도). 라이브 데이터로 위험 0 증명. — ✅ Table (2026-06-30) + ✅ **Stage 1b BigValue/Detail (2026-07-14, 사이클 5 = 🎉 격자 완성)**: `ticker-card`/`indicator-card` 폐기 → `big-value-card`(record, role=primary 강조)/`detail-card`(record, 전 필드) 수렴 — 통합 record descriptor 계약(`recordDescriptors.ts` role 축: primary/badge/secondary/detail, datasource 당 1 pack) + 공유 엔진 훅(`useSingleRecord`/`useReconnectIndicator`). ★ scalar shape 는 record 로 흡수(사용자 확정 — "큰 숫자"는 모양이 아니라 표현 강조, 진짜 단일값 datasource 등장 시 활성. `shapeKind.ts` 판정 박제). **격자 완성 기준 = 데이터-잠금 안티패턴 부채 0** — `kline-chart-card`(TradingView 임베드)는 §5 차트 정책 근거의 **의도된 예외**(특화 렌더러)로 명시 분류. 단일 진실 = `task-record/M2-cycle5-stage1b.md`.
- **Stage 2**: 모양 인식 데이터 레이어 — `series`(범용 history) + `events` 정식화. — ✅ (2026-07-08, 위 2층 게이트).
- **Stage 3**: 새 form (우리 소유 GenericChart = OI/펀딩 history 차트 → 청산 events 시민 → 히트맵/게이지). — ✅ **chart-card 라이브 (2026-07-09)**: uPlot 기반, **series dataShapes 8종**(history 격자 6 + **funding_history 정산 이벤트** + **`liquidation_volume_history` 시간버킷 집계** — M3-step3a `[10-84]`, 2026-07-19: 청산 events 를 series 로 reshape 한 첫 사례. chart form 코드 0줄로 유입 = 직교 실증 3호) — Step 6: 별도 테이블 `history_futures_funding`·interval 축 없음·시간축 funding_time), 호버 툴팁 + interval 토글(registry enum 파생 — 이벤트 datasource 는 자동 숨김) + freshness. **그리기 어휘 line/area/bars/stepped** — bars(부호색+0선)는 이벤트성 series 관례, 오버레이 시 stepped 자동 전환(form 픽셀 정책). feed-card(events, 2026-07-06)와 함께 form 6종. 잔여 = 히트맵/게이지.
- **Stage 4**: AI 계약 — AI 가 {datasource, form, shape, fields, transform} 독립 선택, 레지스트리 파생 검증. — ✅ **완료 (2026-07-12, 사이클 4a+4b — `M2-cycle4b-cross-screener.md §6b`)**: ① **style 축** — 카드 레벨 `style.series`(line/area/bars) override, descriptor 는 default 강등(tone/midline 도메인 가드레일은 직교 불변) ② **스코프 파생 강제** — superRefine 블록(3): `acceptsShapes ⊆ {record,series} × 우리 데이터 레이어가 서빙 × queryableFields` 파생으로 marketType/symbol 필수(series 는 filters `symbol =/in` 대체 = 오버레이 보존, kline 은 자연 면제) → self-correction 작동. **★ M3-step3a 일반화 2건 (2026-07-19)**: 발화 조건 `ds.table` → `isServedByDataLayer = table || fetchKind==="rpc"`(RPC 서빙 datasource 는 table 이 없어 스코프 강제가 통째로 꺼지면 `[10-91]` 계보 재발) / symbol 강제에 **`optionalScopeFields` 면제**(symbol 생략이 "전 시장 집계"라는 **정의된 의미**를 갖는 datasource — 면제는 "강제 안 함"이지 "금지"가 아니라 per-symbol 도 그대로 통과. **marketType 강제는 면제하지 않음** — PK prefix 축은 여전히 필수). 같은 필드를 ChartCard 스코프 가드도 읽어 **단일 진실** 유지 ③ **크로스 metric 자유도** — 통합 `futures_indicators` 렌즈(같은 물리 테이블, 5 family 공유 const + union 등치 불변식) + **dynamicColumns**(표시 컬럼 = AI filters/sort 참조 파생 — 코드 큐레이션 0, CLAUDE.md §카드 기본값·폴백 큐레이션 금지) ④ shape 명시 필드 = **YAGNI 판정**(component+datasource 선택이 shape 를 결정 — 도입 트리거 `[10-106]`). fields 직접 선택(`data.fields`)·transform 은 `[10-106]` 원장.

**경계**: 모양 비호환 조합(scalar→캔들차트)은 무의미(논리 한계). 새 form 1회 제작. 도메인 위생(§"사이트=DB"·sampled·allowlist·단위)은 어떤 form 으로 보여주든 불변.

### 커버리지 매트릭스 — "의미 있는 모양 전부" 실측 (2026-07-21 사용자 확인, living)

> **정의 정밀화 (사용자/CTO 문답 2026-07-21)**: 목표는 "모든 데이터 × 4모양 강제"가 **아니라** "**의미 있는 (데이터 × 모양) 조합은 하나도 막혀 있으면 안 된다**"이다. 일부 조합은 기술 부족이 아니라 논리적으로 무의미(OI 는 사건이 아니라 events 불가 / 심볼 스펙 카탈로그의 series 는 평평한 선). **저장은 항상 1벌**(`_now` 스냅샷 + `_history` 시계열) — shape 는 저장 형식이 아니라 **서빙 시점에 입히는 계약**이며, 같은 물리 데이터가 여러 shape 로 서빙된다(청산 1테이블 → events·set·series 3형태가 그 실증). form 쪽은 자동: 데이터가 어떤 shape 를 서빙하는 순간 그 shape 를 먹는 form 이 전부 열린다(record→big-value+detail / set→table / series→chart / events→feed).

**갱신 규율**: 새 datasource/form/reshape 추가 시 이 표를 갱신한다. 범례 — ✅ 라이브 / ⚠️ 정책상 예외 / 🔳 갭 등재(deferred ID) / 💭 파생 가능·수요 대기 / ➖ 논리적 무의미.

| 데이터 (metric 관점) | record | set | series | events |
|---|---|---|---|---|
| 가격/티커 (현·선물) | ✅ | ✅ | ⚠️ 가격 캔들 = kline(TradingView 임베드, PRD §5 정책 예외). 자체 차트용 가격 series 는 **원료 갭** `[10-88]` | ➖ (체결은 별개 데이터 — 아래) |
| 선물 지표 5족 (OI·LSR×3·taker·basis·펀딩) | ✅ | ✅ (+통합 `futures_indicators` 크로스 스크리너) | ✅ history 6종 + `funding_history` 정산 이벤트 | ➖ 사건 아님 |
| 청산 (forceOrder, 1테이블) | 💭 "최근 N분 요약" 파생 가능 — 수요 대기 | ✅ | ✅ 시간버킷 집계 RPC (M3-step3a/3b — 합계·롱만·숏만·다이버징) | ✅ 피드 |
| 심볼 스펙 (`symbols_meta`) | 🔳 registry 태깅만 존재, 표시 pack 미저작 `[10-112]` | ✅ | ➖ 저갱신 카탈로그 | ➖ |
| 체결/호가 | — **수집 자체 미착수** (ff#3, M3 트랙 ③ — events(체결 tape)·record(호가 스냅샷) 예정 | — | — | — |

**갭의 성격 판정 (핵심 건강 지표)**: "**막힌 칸**(구조가 못 여는 조합) = 현재 0개" — 이것이 2026-07-14 격자 완성 선언의 실체다. 남은 것은 전부 "**빈 칸**"이며 성격이 구조 갭이 아니다: `[10-88]` = 원료(가격 history 적재) 부재 / `[10-112]` = pack 1개 저작이면 끝(수요 대기) / `[10-85]` 예상 청산 히트맵 = 추정 모델 데이터 자체가 없음(형태 문제 아님). 빈 칸을 여는 비용은 청산 집계가 실증했듯 **reshape 배관 1회**(이후 form 코드 0줄) — 유저의 예측 불가 쿼리에 대한 방어는 ① 의미 있는 조합은 격자가 이미 개방 → AI 자율 조립 ② 무의미/미지원 조합은 crash 없이 AI 가 가장 가까운 의미 있는 형태 선택 또는 graceful 안내, 2겹이다.
...
- **`rpcFetch.ts` + `fetchKind` 축** (M3-step3a `[10-84]`, 2026-07-19): datasource 가 데이터를 **"어디서 꺼내나"** 축 신설 — `"table"`(Supabase 테이블 직접 SELECT, 기존 전 datasource 의 기본·유일 경로) vs `"rpc"`(DB 함수 호출). `transport`("어느 길로 닿나": realtime/ws_direct)와 **직교**하며, `transport`/`mergeMode` 와 동형의 **default 있는 가산 필드**라 기존 15 datasource 는 한 줄도 안 바뀐다.
  - **왜 필요했나**: 집계(SUM/GROUP BY)가 필요한 datasource 는 테이블 직접 SELECT 로 서빙할 수 없다. 원본 행을 프론트로 끌어와 집계하면 `initialFetch` 의 `FETCH_HARD_CAP`(3000행)에 걸려 전 시장 24h 청산(24,692행)이 **무증상 절단** — 차트는 정상처럼 보이면서 값만 틀리는 최악의 형태(사이트=DB 위반). DB 집계는 85:1 압축(실측 37ms).
  - **인자 이름을 데이터로 선언**: `rpcSpec = { functionName, argNames }` 가 "fetch 축(코드가 아는 이름) → SQL 인자명" 매핑을 선언한다. 전역 고정 인자 규격을 코드에 박지 않는다(`liveTopicSpec` 원칙 계승) — 미래의 다른 집계 함수가 자기 인자명을 스스로 선언하면 된다. `argNames` 는 자유 record 가 아닌 **축 고정 strict object**(오타가 등록 시점에 안 잡히면 "그 축만 조용히 미전달" = silent-wrong).
  - **경계 규율**: 카드·AI 코드에서 `.rpc()` **직접 호출 금지** — 전부 `rpcFetch` 경유(`initialFetch` 가 확립한 choke point 원칙 그대로, 데이터 레이어 교체 시 이주 지점 단일화). `seriesFetch` 가 `fetchKind` 로 분기하며 table 경로는 무변경.
  - **실패 표현 대칭**: RPC 에러는 빈 배열이 아니라 **throw** — 빈 배열로 흡수하면 "함수 미배포/권한/인자 불일치"가 화면에 "데이터 없음"으로 위장돼 table 경로(`initialFetch` throw → "chart data error")와 비대칭이 생긴다. graceful 은 "crash 0"이지 "실패를 정상으로 표시"가 아니다.
  - **`optionalScopeFields`**: symbol 생략이 "전 시장 집계"라는 정의된 의미를 갖는 축 선언. 훅은 registry 를 모르는 기계로 유지하고, 상위(ChartCard)가 이 파생값을 `allowEmptySymbols` 로 명시 전달한다. ⚠️ 라이브 적발 교훈 — 이 옵션 없이는 `seriesFetch` 의 전 시장 분기가 **도달 불가**였다(훅이 빈 symbols 를 그 앞에서 idle 로 끊음). 계층 경계를 넘는 능력 추가는 **양쪽 가드를 함께** 봐야 한다.
- **`useDataServiceSeries.ts`** (Composable Stage 2 Step 2, 2026-07-08): 네 번째 훅 — **시계열(series shape) 주기 pull**. `history_*` 테이블은 Realtime push 가 없어(PRD §3 "`_history` 기반 카드의 주기적 갱신") 훅이 refreshIntervalMs 로 스스로 재fetch 하는 **첫 주기 pull 구현** — 경로 B 의 읽기 절반(프론트→Supabase SELECT, Realtime 미경유). 다중 심볼 오버레이 = **per-symbol 병렬 쿼리**(`seriesFetch.ts` — PK prefix 정합 7ms; `symbol IN`+글로벌 정렬은 600만행 스캔 500ms 디스크 폭탄이라 **금지**, backend-infra EXPLAIN 실측 2026-07-08). oldest-first 훅 보증 / 호출자 콜백 0개 / soft-hard 실패 분리(재fetch 실패·의심스러운 0행 전환은 기존 곡선 유지 + `lastUpdatedAt` 미전진) / per-series 참조 재사용(무변화 곡선 = 이전 참조 → uPlot setData 절약). 첫 소비자 = GenericChart(사이클 2 Step 4).
  - **절대 시간창 `fromIso/toIso`** (M3-step3b `[10-120]`②, 2026-07-21): AI filters 의 시간축 범위 연산자를 카드 층(`useChartScope.resolveChartTimeRange`)이 번역해 관통 — rpc 경로 = `p_from/p_to` 인자, table 경로 = `gte/lte` RangeFilter(history 6종의 "광고 ↔ 미배선" 선재 갭 해소). 명시 `fromIso` > `lookbackMs` 파생. ⚠️ 두 경로 모두 "창 안 최신 N"(DESC limit)이라 창 > limit×interval 이면 앞부분이 잘린다 — **무음 금지**: ChartCard 가 `isSeriesWindowTrimmed` 로 감지해 "window trimmed" 고지(reviewer W1).
  - **성분분해(breakdown) 계약** (M3-step3b `[10-121]`, 2026-07-21): 한 집계의 도메인 성분(롱/숏 청산액)을 한 차트에 대향 표시. **AI 계약 = `style.breakdown`("total"|"components") + `filters side`(subset 축)** / **descriptor = 성분의 의미**(컬럼·라벨·시장영향 방향색·invert 대향)만 선언. `resolvePlotSpecs` 진리표(4b dynamicColumns 의 차트판): side 필터→단일(pushdown 이 이미 좁힘) / 오버레이→total 폴백(4시리즈 판독 불가 — form 픽셀 정책, 스키마 describe 에 광고) / AI 명시 ?? 도메인 기본. invert 는 **표시층 부호 반전**일 뿐 — null 은 null(gap), 툴팁·축은 원값/절대값 복원("음수 청산액" 거짓 함의 차단). ★교훈: 코드 소유 도메인 기본값(생략=다이버징)이 AI 의 "생략" 자연 의미론을 뒤집을 때는 **그 기본을 datasource description 에 사실로 고지**해야 한다 — 안 하면 AI 가 "생략=total"로 합리 추론해 제목↔렌더 불일치(라이브 실측·서술 고지로 해소).
...
#### 경로 A (WS 프론트 직결) 인프라 (M2 경로 A 테마, ✅ **완료 2026-06-24** — Step 4 Phase B 라이브 플립. ticker 가 경로 A 직결 가동 = PRD 3대 경로 전부 구현)

PRD §2 경로 A(거래소 WS → 워커 WS 서버 → 프론트 직결, Supabase 미경유)의 프론트 측. **라이브 가동 중**: now_spot/futures_ticker `transport:"ws_direct"` 플립(2026-06-24) → 가격이 DB 안 거치고 ~1초 매끄럽게 갱신("박동" 소멸). transport 미지정 datasource(funding/OI/LSR 등)는 그대로 경로 B 폴백(공존). channelManager(경로 B)의 검증된 패턴을 미러:
...
- **인증**: 프론트가 Supabase access token 을 `Sec-WebSocket-Protocol` subprotocol(`[travis-live-v1, <jwt>]`)로 핸드셰이크에 실음 → 워커가 **ES256 비대칭 검증 (JWKS)** (`createSupabaseTokenVerifier` = `createRemoteJWKSet(<SUPABASE_URL>/auth/v1/.well-known/jwks.json)`, `jose`, algorithms `["ES256"]` 고정·aud=authenticated·exp). 무효/만료/위조는 `verifyClient` 에서 upgrade 거부(WS 미수립). 만료 시 close(4401) → 재인증. ★ **ES256/JWKS 채택 (Step 4 Phase B 라이브 정정 2026-06-24)**: Step 2 는 HS256 대칭키였으나, 라이브 B-3 에서 이 Supabase 프로젝트가 **이미 비대칭 ES256(ECC P-256) 서명으로 마이그레이션**(Legacy HS256=verify-only)됨이 드러나 HS256 검증기가 실토큰을 전량 `malformed` 거부 → JWKS 공개키 ES256 검증으로 전환. **공개키만 보유 = 위조 불가**(HS256 대칭=검증자가 서명도 가능 대비 blast radius 축소). security-auditor 0C/3W/8P. (`feedback_external_api_live_smoke` 사례.)
- **fail-closed**: `SUPABASE_URL` 미설정 시 WS 서버 미기동(인증 없는 노출 0초) — 단 수집(경로 B)은 무관하게 동작. (JWKS 검증 키를 SUPABASE_URL 에서 받으므로 게이트도 SUPABASE_URL. 구 `SUPABASE_JWT_SECRET` 은 폐기.)
...
**책임 경계**: `packages/data-service` 는 worker 의 bulk write + AI 오케스트레이터의 read 추상. `apps/web/lib/dataService` 는 프론트 카드의 Realtime 구독 fan-out + 단일 channel 관리(경로 B) **+ WS 직결 토픽 fan-out(경로 A, ✅ 라이브)**. 두 layer 가 같은 `Database` 제네릭 타입은 공유하지만 인스턴스 / 인증 컨텍스트 / 호출 패턴 모두 분리.
