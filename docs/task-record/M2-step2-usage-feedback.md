...
| **경로 A — WS 프론트 직결** (PRD 3 데이터 경로) | (미계획) | ★ 테마 A 완결 후 실측 (2026-06-11): flash 가 "박동" — 경로 B 1~2초 뭉텅이 구조 한계. liveness 의 나머지 절반 | **★ 4회전 — 🔄 진행 중 (2026-06-22, 토대 완성)**. Step 1+3a+3b ✅(워커 WS 서버+레지스트리 transport 계약+프론트 라우터 휴면). ▶ 도메인→Step 2(wss)→Step 4(플립 박동소멸 검증). 단일 진실 `M2-pathA-ws-direct.md` |
