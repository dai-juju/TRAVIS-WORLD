// ============================================================
// Binance USDT-M Futures REST 어댑터 (M1.3 Step 3c + 3e).
//
// 배치 가능 (3c):
...
export class BinanceUsdmAdapter {
   ...
   * last_funding_rate, interest_rate, next_funding_time을 한 방에 수집.
   * 반환은 indicator의 premium 도메인 컬럼만 포함.
   */
  async fetchPremiumIndex(): Promise<FetchResult<NowFuturesIndicatorInsert[]>> {
    ...
    if (!res.success) return res;
    return { success: true, data: res.data.map(normalizeUsdmPremium) };
  }
