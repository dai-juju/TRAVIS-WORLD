...
> **상태**: ✅ **Stage 1 완료 (Step 1~5, 2026-06-30)** — `table-card` 가 옛 카드 2종(`coin-list-card`/`indicator-list-card`)을 **라이브 대체**. 게이트 registry 권한 일원화 · alias 대신 저장뷰 삭제 · all-view 일관화(`defaultLimit` 필드 제거). **라이브 G2 PASS**: top gainers/all spot(449 가상스크롤)/**top OI=옛 F3 "realtime error" 해소(site=DB 자릿수 일치)**/funding + 단일/다중 의도분리(zod S2 해소) + COINM 단위 + 콘솔 0. 커밋 `2a964bd`(Step 3+4)·`f1221ae`(docs)·all-view 수정(이 세션 커밋). **사이클 1(`[10-77]`+`[10-82]`) ✅ 배포 완료 + G1·G2 PASS (2026-07-07, G3 usage 관측만 잔여 — `M2-[10-77]-realtime-throttle.md`). ▶ 사이클 2 GenericChart 🔄 착수 (2026-07-08)** — 계획 승인(6-step, 사용자 확정 4건: 착수/펀딩 후반 Step/오버레이=(a) 다중 심볼만·(b) 이중축=Stage 4 펜싱/uPlot) + **Step 1(Shape 계약 정식화) ✅ 당일 완료**(servableShapes×acceptsShapes 2층 게이트 — 렌더 게이트 무변경 byte-identical, 불변식 11종). **단일 진실 = `M2-cycle2-genericchart.md`** (§11 항목 4 의 실행체). **다음 = 사이클 2 Step 2 (`useDataServiceSeries` 훅).**
