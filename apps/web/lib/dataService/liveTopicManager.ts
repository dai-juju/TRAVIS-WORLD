...
function mapStatus(s: LiveConnectionStatus): LiveStatus {
  switch (s) {
    case "open":
      return "subscribed";
    case "connecting":
...
class LiveTopicManager {
  ...
  subscribe<T extends Record<string, unknown>>(
    ...
    if (listener.onStatus) {
      try {
        listener.onStatus(mapStatus(conn.getStatus()));
      } catch (err) {
      ...
      this.connection = new LiveConnection(
        {
          ...
          onStatus: (s) => this.broadcastStatus(mapStatus(s)),
  ...
  __resetForTesting(wsFactory?: WebSocketFactory, url?: string): void {
    for (const entry of this.entries.values()) {
