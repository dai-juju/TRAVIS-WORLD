...
| | `last_settled_funding_rate` (M1.8 §8.1 ✅ 신설 2026-05-25 / **★N1 소스 교체 2026-07-10**) | Realized last settled funding rate — raw decimal. 정산 직후 4h(또는 8h) 동안 같은 값 고정. **Source = `/fapi/v1/fundingRate` 최신 element — collector-history `fundingHistoryTask` 가 cycle 끝(20분)에 `lastSettledReflection` 으로 반영** (커밋 `c2515ae`). ⚠️ 구 소스 `premiumIndex.lastFundingRate` 는 이름과 달리 **predicted 스냅샷**(네이밍 트랩 — 실측 카드 +0.00620% vs 공식 +0.00906%, 사이클 2 G2 적발)이라 폐기. predicted 와 시간축이 다른 별개 metric. |
| | `last_settled_funding_time` (M1.8 §8.1 ✅ 신설 2026-05-25, BIGINT / **★N1 소스 교체 2026-07-10**) — **`/fapi/v1/fundingRate.fundingTime` 실측값**을 collector-history 가 rate 와 **같은 정산 이벤트 한 쌍**으로 반영 (부분 교체는 mismatched pair 위험이라 둘 다 이동). countdown 계산 보조 데이터. (이력: ~2026-07-10 은 premiumIndexTask 가 `nextFundingTime - fundingIntervalHours × 3600 × 1000` D15 역산으로 30분 폴링 채움이었음 — 역산 자체는 정확했으나 rate 쪽 predicted 오염과 함께 소스 일원화.) |
