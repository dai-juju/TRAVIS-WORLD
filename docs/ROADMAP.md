...
> **▶ 현재 위치 (2026-06-28) — 확장 루프 5회전 = 경로 A fast-follow 트랙**: ① **ff#1(마크가격/펀딩) ✅** (2026-06-26, IndicatorCard 개조, 라이브 G2 5게이트) → ② **ff#2(청산 피드 카드) 🔄 — Phase A non-web 전부 ✅ + Step 4(`useDataServiceFeed` 훅 = `content` updateMode 첫 실사용, 불변식 A~F) ✅** (Step 1 결정 4건 + 3a 토픽 keystone + 3b liquidation 플립 + 2 워커 publish + 4 피드 훅, 전부 transport 휴면=화면 변화 0). `[10-71]` 회수(web lint 첫 부팅 → 잠복 `react-hooks/refs` 22건 IndicatorCard/TickerCard 근본 수정=render-phase setState) + `[10-73]` 신설(filter forward-application). 커밋 `2870e63`(push main). **▶▶ /clear 후 다음 = ff#2 Step 5(`LiquidationFeedCard` + 컴포넌트 등록 + aiCardConfig refine 일반화)** → Step 6(Phase B 라이브 플립+G2) → 그 후 ff#3(trade+호가)/새 테마. 상세 ff#2 트래커 = 본 §경로 A fast-follow #2 (아래) + 단일 진실 `docs/task-record/M2-pathA-ff2-liquidation.md`, 메모리 `project_m2_pathA_ff2`.
>
> **▶ 이전 위치 (2026-06-24) — 확장 루프 4회전 = 경로 A (WS 프론트 직결) ✅ 완료 = 🎉 PRD 3대 데이터 경로 전부 구현**: Step 4 Phase B 라이브 플립으로 ticker 가 경로 A 직결 가동(박동 소멸 사용자 실측 + site=DB 소수점 일치 + 토큰 통과). ★ 라이브 정정 = 이 Supabase 가 이미 ES256 비대칭 서명 → 워커 JWKS 공개키 검증 전환(Step 2 HS256 가정 정정, `feedback_external_api_live_smoke`). 커밋 `f074ce1`/`d1a0dae`/`ecdcaa4`/`3c05a37`/`3886334`. `[10-1]`(a)·`[10-53]` 묘비, 신규 `[10-60]`~`[10-67]`. **▶▶ 다음 (사용자 결정 `/clear` 후) = 경로 A fast-follow 3종 순차(① funding/마크 경로A → ② 청산 피드 → ③ trade+호가) → 그 후 새 테마.** 단일 진실 `docs/task-record/M2-pathA-ws-direct.md §3` + 본 §경로 A 테마 완결 블록. 메모리 `project_m2_pathA_complete`.
>
...
>
> **★ ✅ 완결 (2026-07-06) — 확장 루프 6회전 "Feed form + 청산" = Composable Stage 3 events 첫 시민 = ff#2 재개·완성**: 모양-제네릭 `feed-card`(events 2번째 form) + 청산×table-card 8번째 datasource(같은 데이터 두 형태, AI 자율 분기 라이브 실측) + 피드 과거 seed + CM migration 오염 당일 해소. 라이브 G2 전 게이트 PASS. **다음 = ③ Realtime throttle `[10-77]` → ④ GenericChart(Stage 2 series)/Stage 4.** 단일 진실 `docs/task-record/M2-pathA-ff2-liquidation.md`.

##### Stage 1 Steps (2026-06-28 `@roadmap-milestone-manager` 분해) — 스냅샷 Table 통합 · 위험 0 증명 · 예상 13~19시간

> **🔄 진행 (2026-06-29)**: **Step 1 ✅(`47ee8e5`) + Step 2 ✅(`5f4574f`)** — `table-card` 레시피(`tableDescriptors.ts`)+컴포넌트(`TableCard.tsx`+`tableCardFormat.ts`) 완성, **미등록=화면 무변경**. 신규 28 test + 전체 web 338 회귀 0 + `@code-reviewer` 0C ×2. 색 계약=tone(방향)/intensity(크기 0..1) 분리(픽셀 바닥값 form 소유). W1(문자열 sort 회귀) `compareByField` 추출 수정. 부채 `[10-74]`(descriptor 3중 수렴, 🟢)·`[10-75]`(지표 가상화 컬럼폭, 🟡 Step 5 튜닝).
>
> **★ Stage 1 ✅ 완료 (Step 1~5, 2026-06-30)** — `table-card` 가 옛 카드 2종을 라이브 대체(Step 3+4 `2a964bd` + Step 5 G2 라이브 PASS + all-view 수정). 게이트 registry 권한 일원화 · alias 대신 저장뷰 삭제 · all-view 일관화(`defaultLimit` 제거 + `[10-75]` 해소). **G2**: top OI=옛 F3 "realtime error" 해소(site=DB) + 단일/다중 의도분리(zod S2) + COINM 단위 + 콘솔 0. type-check 6패키지 clean + shared 61/web 334 회귀 0 + ESLint exit 0 + 자문 4종(nextjs/code-reviewer/zod/security) **0 Critical**. zod W1(requiredFields 무결성 불변식)·code-reviewer W1/W2(stale 주석) 반영, W3(파일 분할)=`[10-76]` 연기. 다음 = **확정 순서(사용자 2026-06-30): ② Feed form+청산(=ff#2 재개=Stage 3 events 시민, 옛 LiquidationFeedCard→모양-제네릭 Feed form=안티패턴 회피, ff#2 자산 재사용) → ③ Realtime throttle `[10-77]` → ④ GenericChart/Stage 4**. 단일 진실 `M2-composable-expressiveness.md §10·§11`.
>
> (이력) **사용자 결정 (2026-06-29): Step 3+4 를 한 세션에 묶어 진행.** 분리 시 Step 3(등록=AI 가 `table-card` 선택 가능) 후 옛 카드와 새 카드를 AI 가 겹쳐 고르는 **모호 공존 구간** 발생(둘 다 정상 렌더하나 비결정적) → **등록·옛카드 폐기·alias 를 한 번에** 처리해 공존 창 제거. **다음 세션 = Step 3+4 통합 → 그 후 Step 5 라이브 G2.** (Step 3·4 세부는 아래 항목 그대로, 한 커밋/세션으로 묶음.) 단일 진실 = `docs/task-record/M2-composable-expressiveness.md §10`. ⚠️ 아래 Step 3·4 본문은 Step 1·2 이전 분해라 일부 함수명(`getIndicatorListDescriptor` 등)·세부가 stale — **현행 함수/파일/id 는 `§10 ▶ 다음 세션` 표현이 우선**(예: 신규 표시 lookup = `getTableDescriptor`, 게이트 = `isDatasourceSupportedByComponent`).

> **범위 확정**: `coin-list-card`(ticker) + `indicator-list-card`(지표 5종) → 모양-제네릭 `table-card` 한 컴포넌트로 통합. 라이브 데이터 7 datasource(ticker 2 + indicator 5)로 회귀 0 증명. **prove-first**: `shape` 필드 미도입(현 `dataShapes` 허용목록 유지), descriptor 가 Stage 2 `shape` 끼움 대비. **scope 제외**: BigValue/Detail 일반화(별도) · GenericChart · series 훅 · 청산(events) · kline · OKX. 단일 진실 = `docs/task-record/M2-composable-expressiveness.md`.

- [x] **Step 1 ✅ (2026-06-29, commit `47ee8e5`) — 통합 table descriptor 계약 설계 + ticker descriptor 팩 신설 (데이터 레이어, 화면 무변경)** — 목표: 기존 `IndicatorListDescriptor`/`ListColumn` 을 **7 datasource 어떤 것이든** 표로 구동하도록 확장(identity/label 컬럼=symbol 명시화 · pk 필드 · defaultSort · defaultLimit · fetchAll/가상화 정책을 descriptor 가 공급) + `tone` 계약을 3값 enum→**연속 intensity 표현 가능**하게 확장(ticker 의 `|pct|/10` opacity 색농도를 인라인이 아닌 descriptor 파생으로). 확장 형태는 **Stage 2 `shape` 필드가 재작업 없이 끼워지도록** 순수 표시+fetch 정책 메타로 한정(shape-agnostic). + `ticker` descriptor 신설(now_spot_ticker·now_futures_ticker, CoinListCard 3컬럼 symbol/last_price/pct 복제). — 산출물: ✏️ `apps/web/lib/cards/indicatorListDescriptors.ts`(계약 확장 또는 `tableDescriptors.ts` 로 승격) · ➕ ticker descriptor · ✏️ 관련 타입 — 검증: `pnpm -r type-check`·`lint` green + 단위 테스트(ticker 2 datasource 의 `columns[].key ∈ queryableFields` · tone/intensity 함수가 샘플 row 에 기대값 반환). 컴포넌트는 아직 미소비 = 순수 데이터 레이어. — 의존성: 없음(착수 지점) — 예상: 3~4시간 — **순서 근거**: 표(form)가 무엇을 그릴지는 "설계도(descriptor)"가 정한다 → 컴포넌트보다 설계도가 먼저. 비전공자 비유: "표 틀을 짜기 전에, 각 데이터를 어떤 칸·색·단위로 보여줄지 적은 '레시피 카드'부터 만든다."

- [x] **Step 2 ✅ (2026-06-29, commit `5f4574f`) — 모양-제네릭 `TableCard` 컴포넌트 신설 (IndicatorListCard 엔진 base + CoinListCard 능력 흡수, 미등록)** — 목표: IndicatorListCard 의 검증된 descriptor 구동 thead/tbody 엔진을 base 로 새 `TableCard.tsx` 작성하되, **CoinListCard 고유 능력 3종 무회귀 흡수**: (a) `@tanstack/react-virtual` >100행 가상화 + 가상화↔FLIP 상호배제 분기, (b) 서버 필터 pushdown + fetchAll/`.range()` "all coins", (c) 연속 색농도(Step 1 descriptor 파생). 전부 descriptor 구동 — 인라인 ticker 컬럼 0 · 하드코딩 기본정렬 0. — 산출물: ➕ `apps/web/components/cards/TableCard.tsx` · ✏️ `__tests__` — 검증: type-check·lint green + 렌더/단위 테스트(ticker descriptor·indicator descriptor 각각 올바른 컬럼 산출 · 가상화 임계값 분기 커버). 아직 미등록 = 고립 빌드. — 의존성: Step 1 — 예상: 4~5시간 — **순서 근거**: 설계도(Step 1)가 있어야 그걸 읽는 표 컴포넌트를 만들 수 있다. 비유: "레시피가 완성돼야 그 레시피대로 요리하는 주방을 짓는다."

- [x] **Step 3 ✅ (2026-06-30) — self-gate 단일화 + dataShapes 7-union + 듀얼 등록 (`table-card` id)** — 목표: 새 컴포넌트 `table-card` 를 **양쪽 레지스트리 동일 id 등록**(`defaults.ts` componentRegistry: dataShapes=7 datasource union + 병합 description / `registerCards.ts`+`cardComponentRegistry.ts`: React 맵→TableCard) — `feedback_registry_react_ai_sync` 불변식. **self-gate 2진실→1진실 통합**: 렌더 가능성은 registry `dataShapes` 단일 출처(`renderableDatasource.ts`)로, `getIndicatorListDescriptor` 는 게이트 역할 폐기→순수 표시 lookup 으로 강등. superRefine 이 dataShapes 파생이라 7 union 만으로 AI 자동 허용(스키마 변경 0). — 산출물: ✏️ `packages/shared/src/registries/defaults.ts` · ✏️ `apps/web/lib/registerCards.ts` · ✏️ `apps/web/lib/cards/renderableDatasource.ts`(필요 시) · ✏️ drift 테스트(`indicatorListDescriptors.test.ts` ticker=descriptor 없음 단언 **뒤집기** + `columns[].key↔queryableFields` 불변식 7종 확장) — 검증: type-check·lint·test green + superRefine 이 7종 전부 자동 허용(schema 테스트) + `isDatasourceSupportedByComponent("table-card", x)` 7종 true + buildSystemPrompt 에 table-card 노출 + 옛 카드 회귀 0(아직 공존). — 의존성: Step 2 — 예상: 2~3시간 — **순서 근거**: 표 컴포넌트가 존재해야 시스템에 "정식 등록"할 수 있다. 비유: "주방을 다 지은 뒤에야 메뉴판에 새 요리를 올린다."

- [x] **Step 4 ✅ (2026-06-30, ★ alias 대신 저장뷰 삭제) — 옛 카드 폐기 + 프롬프트 전환** — 목표: `coin-list-card`/`indicator-list-card` 등록 제거(또는 table-card alias) + 옛 componentId 참조 저장 뷰를 table-card 로 **로드 시 alias resolver**(권장: graceful·가역) 또는 1회성 data migration(본인 단독 사용=위험 낮음) + buildSystemPrompt 예시 교체 + 옛 컴포넌트 파일 정리(즉시 삭제 또는 thin alias re-export). — 산출물: ✏️ `defaults.ts`·`registerCards.ts`(옛 id 제거/alias) · ➕/✏️ alias resolver · ✏️ `buildSystemPrompt.ts` · ✏️ `serialize.test.ts`(옛 id→table-card 왕복) · 옛 카드 파일 — 검증: type-check·lint·test green + 옛 saved view(coin-list-card 포함) 로드→table-card 로 해석·렌더 + 옛 id 잔존 참조 0(alias 경유만). — 의존성: Step 3 — 예상: 2~3시간 — **순서 근거**: 새 id 가 라이브여야 옛 id 를 안전하게 그 위로 옮긴다. 비유: "새 메뉴가 손님에게 나가는 걸 확인한 뒤에야 옛 메뉴를 내리고, 옛 주문서가 새 메뉴로 자동 번역되게 다리를 놓는다."

- [x] **Step 5 ✅ (2026-06-30, 라이브 PASS + all-view 수정) — 라이브 G2 검증 (7 datasource site=DB + 회귀 0)** — 목표: 라이브로 7종 전부 띄워 검증 — ticker(spot+futures: top gainers · all-coins 1,447행 가상 스크롤 · 연속 색농도) + 지표 5종(funding/basis/OI/LSR/taker 랭킹). site=DB 일치 · 박동 무회귀 · 가상화↔FLIP 정합 · watch 컬럼 flash · 옛 능력 보존(all-coins fetchAll·소규모 FLIP). — 산출물: ✏️ `docs/task-record/M2-composable-expressiveness.md`(Stage 1 ✅) · ✏️ `docs/deferred-task.md` · ✏️ ROADMAP 본 섹션 — 검증: G2 체크리스트 all green + Playwright DOM 행수 교차검증 + crypto-trader UX 자문 + code-reviewer 0 Critical. — 의존성: Step 4 — 예상: 2~3시간 — **순서 근거**: 모든 배선이 끝난 뒤 실데이터로 진실 검증. 비유: "주방·메뉴판·번역 다리가 다 됐으니, 실제 손님 7명에게 내보내 맛(숫자 정확성)과 옛 단골 경험이 안 깨졌나 최종 확인."

> **Stage 1 scope 차단선**: ❌ `shape` 필드 본 도입(=Stage 2 첫 작업) ❌ BigValue/Detail 일반화(ticker-card→BigValue·indicator-card→Detail = 별도 분해, descriptor 구조적 상이) ❌ GenericChart / series 훅(Stage 3) ❌ 청산(events, ff#2 재개 시) ❌ kline(TradingView 자체 fetch) ❌ OKX/타 거래소. **deferred 후보**: tone 계약 확장의 단일심볼 descriptor(indicatorDescriptors.ts) 미래 정합(`[10-74]`). **[✅ 결정됨 2026-06-30]** saved_views = alias 대신 **전체 삭제**(베타 전) · 지표 defaultLimit = **필드 완전 제거**(all-view 일관 = 생략=전부·숫자=그수, `[10-75]` 해소).
...
##### Steps (2026-06-27 분해) · Phase A(1~5 코드·휴면) / Phase B(6 라이브, 사용자 협업) — **★ ✅ 완결 (2026-07-05~06, 커밋 `208e412`→`aad6891` 총 9개) = 확장 루프 6회전 "Feed form + 청산" = Composable Stage 3 events 첫 시민 = ff#2 재개·완성.** Step 1·3a·3b·2·4 ✅(2026-06-28) + Step 5 **재정의 완수 = 모양-제네릭 `feed-card`**(옛 LiquidationFeedCard 폐기 — 데이터-잠금 안티패턴 회피) + Step 6 라이브 G2 PASS + Step 7 피드 과거 seed. **다음(사용자 확정 순서) = ③ Realtime throttle `[10-77]` → ④ GenericChart(Stage 2 series)/Stage 4 AI 계약.** 단일 진실 `docs/task-record/M2-pathA-ff2-liquidation.md`.
...
- [x] **Step 5 ✅ (2026-07-05~06, ★ 재정의됨 = 모양-제네릭 `feed-card`) — 2번째 form(events shape) 신설 + 청산×table 8번째 datasource** — ★ 옛 "LiquidationFeedCard"(청산 전용 데이터-잠금 카드) **폐기** → CLAUDE.md §최상위 개발 축(Form↔Data 직교) 준수하여 **모양-제네릭 `feed-card`**(events shape 를 받는 어떤 datasource 든 렌더하는 2번째 form)로 재정의·구현. 결과 registry 컴포넌트 5종(ticker/table/kline/indicator/**feed**). **같은 청산 데이터를 두 형태로**: feed-card(events 흐름) + **table-card 8번째 datasource**(스냅샷 랭킹) — AI 가 쿼리 의도로 자율 분기("watch"→feed / "biggest"→table) 라이브 실측 확인. refine 일반화(`subscribesByTopic` — frozen 카드 스키마 차단) + notional(USD) DB 컬럼+방송(USDM `z×ap`·COINM `zEff×contractSize`) + 범위 연산자 서버 pushdown 신설 + maxRows 상한. — 검증: type-check·lint·test green + 빈 피드 graceful + registry 파생 가드(양쪽 등록) + AI form 자율 분기 라이브 확인 — 자문: nextjs-frontend·zod·security-auditor·crypto-domain — 의존성: Step 4

- [x] **Step 6 ✅ (2026-07-05~06, 라이브 G2 전 게이트 PASS) — Phase B 라이브 플립 + G2** — 워커 재배포 + `transport:ws_direct` 플립 → 라이브 검증: **site=DB 삼중 일치**(tape 3행 공식 재계산 + biggest 8행 + ETH seed 15행) + sampled subtitle AI 자작 + **form 자율 분기**(watch→feed / biggest→table) + 콘솔 무해 2건. **🔴 사고 당일 해소**: Binance CM migration(effective 6-30)으로 `!forceOrder@arr` 가 UM+CM 병합 → COINM 라벨 오염 21.9만 행 → crypto-domain 자문으로 당일 규명(`st` 필드=권위 판별자, `[10-14]` 감시 적중) + **2단 가드**(st 우선 + 멤버십 폴백) + 오염 220k DELETE + 재오염 0 실측. — 검증: G2 전 게이트 green + WS 재연결 graceful + 기존 경로 A/B 카드 무중단 공존 — 자문: crypto-trader·code-reviewer·security-auditor·crypto-domain — ff#2 잔여 `[10-72]` 회수 / `[10-73]` YAGNI 묘비 / 신규 deferred `[10-78]`~`[10-82]`.
- [x] **Step 7 ✅ (2026-07-05~06, 사용자 결정) — 피드 과거 seed (훅 불변식 G)** — feed-card 구독 시작 시 빈 화면 대신 최근 청산 이벤트를 DB 에서 한 번 seed 로 채워 넣음(사용자 결정). `useDataServiceFeed` 불변식 G 로 편입 — 실시간 이벤트 도착 전에도 과거 tape 로 즉시 유의미. G2 에서 ETH seed 15행 site=DB 일치로 검증.
...
## ▶ 다음 2-사이클 확정 (2026-07-07 계획 세션, 사용자 확정)

> ff#2(Feed form + 청산) ✅ 완결 후 다음 순서. 단일 진실 = `docs/task-record/M2-composable-expressiveness.md §11`(갱신됨).

- ~~**사이클 1 — `[10-77]` Realtime throttle + `[10-82]` favicon**~~ ✅ **완결 (2026-07-09 G3 PASS = `[10-77]` 묘비)**: `MarkPriceWriteCoalescer` 60초 write coalescing 라이브(경로 A 1초 무영향) + G1·G2 PASS + **G3 PASS**(Realtime 1.41M/5M 28%·일별 피크 473K→~100K 하향·favicon 404 소멸·deadlock 0). 신규 `[10-86]`(ticker churn 후보 — 현 usage 수준상 비긴급). 단일 진실 = `task-record/M2-[10-77]-realtime-throttle.md`.
- **사이클 2 — GenericChart (Composable Stage 2+3)** ✅ **완결 (2026-07-10 — Step 6 라이브 G2 7종 전부 PASS: site=DB Binance 공식 API 3연속 8자리 일치 + AI 분기 log_chat 8/8 + 증분 수집 첫 실전 PASS. G2 적발 결함 3건 당일 수정 = ★N1 last_settled=predicted 네이밍 트랩[M1.8 잠복 위생 #9, worker→collector 소스 교체 `c2515ae`] + RF 줌 툴팁 오스냅 + y축 부호 소실[`ec4bb06`, `[10-92]` 회수 동반]. 상세 §4i. 다음 사이클 = `[10-35]` lag[사용자 확정 2026-07-10, Stage 4 선행]). 단일 진실 = `task-record/M2-cycle2-genericchart.md`**. (이하 진행 이력) ① Shape 계약(2층 게이트) ② `useDataServiceSeries`(per-symbol 병렬 7ms vs `IN` 500ms 실측) ③ history datasource 6종 + chartDescriptors ④ GenericChart form(uPlot 1.6.32) ⑤ **chart-card 등록 + 원자적 일괄 push + 라이브 G2 전 게이트 PASS (2026-07-09)** — AI 자율 분기 5/5(trend→chart/top→table/funding→indicator/watch→feed + ★"compare" 시간축 단서 유무로 table↔chart 갈림 = 직교 실증) · limit=시간범위 역산(24h@5m→288) · site=DB 모양 일치 · **라이브 hotfix 4연쇄**(marketType 500→registry 파생 가드 / 카드 축소 되먹임→absolute 격리 / RO 소진→재-observe / ★uPlot.min.css 누락=DPR 1.25 잘림 근본 — 전부 개발환경 불가시 부류) + **UIUX 확장**(사용자 결정 4건: 플로팅 툴팁·interval 토글 9종 registry 파생·포인트수 유지·freshness "last point"). 신규 `[10-91]`~`[10-93]`. ⑥ **Step 6 펀딩 코드+데이터 ✅ (2026-07-09, §4h)**: ★도메인 자문이 원안("기존 컬럼 채움") 기각 → **별도 이벤트 테이블 `history_futures_funding`**(정산 1회=1행, interval 없음, indicator 죽은 컬럼 2 DROP) + funding rate-limit 버킷 80/min(Plan 검증이 "무음 무제한" 결함 적발) + USDM symbol-생략 전역 페이지네이션 **backfill 완주 196,600행/691심볼/60일**(라이브 smoke 가 동시정산 페이지 절단 유실 적발→수정) + COINM 3,580행 + **chart-card bars/stepped 어휘**(펀딩 전용 카드 아님 — disp.fill 부호색·오버레이 stepped 전환·AI interval 오염 가드). reviewer 2회 0C. 신규 `[10-94]`~`[10-96]`. **▶ 잔여 = Step 6 별도 라이브 G2 만(§4h 끝 게이트 7종) → 통과 시 사이클 2 완결**. 오버레이 = (a) 다중 심볼만((b) 이중축 = Stage 4 펜싱 / (c) 가격 = `[10-88]` 데이터 갭). 청산 집계 배관 = `[10-84]` 후속.
- ~~**사이클 3 — `[10-35]` forward-fill lag 해소**~~ ✅ **완결 (2026-07-12, 단일 진실 = `task-record/M2-[10-35]-forward-fill-lag.md` §4d/§4e)**: 원인 확정 = `perTaskReqPerMin 150÷6=25/min` 정적 분할이 steady-state 예산 ~83% stranded(전역 token-bucket `[8-31]`ⓐ 도입 이전 잔재 — cycle 하한 8.4h ≈ 관측 8.6h 정합). **레버 1(분할 제거, `bf203ff`)만으로 정상상태 실측 5m 219→91분(최악 8.6h→~1.5h) / 30m 404→13분 / 1h 554→73분** + 안전 게이트(비-basis -1003 전후 0 / 429 baseline 질감). **Step 3 사용자 게이트 "충분"(2026-07-12) → 레버 2 = `[10-103]` deferred 강등 / `[8-31]`ⓓ = 별개 복원력 축 존치.**
- **사이클 4 — Stage 4 AI 계약 + 쿼리 자유도 묶음 (사용자 확정 2026-07-12, 테마급 실측 13~19h → 4a/4b 분리)**: 배경 = **PRD §2 "쿼리 자유도" 명문화(2026-07-12 사용자 재확정 — 모든 데이터 × 모든 form × 스타일, "유저의 어떤 요청이든")**.
  - ~~**4a — `[10-101]` 차트 스타일 자유**~~ ✅ **완결 (2026-07-12 당일, 단일 진실 = `task-record/M2-cycle4a-chart-style.md`)**: AI 계약 카드 레벨 `style:{series:"line"|"area"|"bars"}` + descriptor=default 강등 + ChartCard 파생 descriptor(tone/midline 가드레일 불변) + 프롬프트 다이얼(하드코딩 0). **라이브 G2-a 전 게이트 PASS**("simple line chart" → 선 차트 + log_chat 정확 emit / 미지정 = 기본 bars 대조 / OI as bars = AI 카드 2장 분리 자율 판단 + 모노크롬 유지). G2-a 적발 `[10-104]`(symbol+filters 이중 지정 오버레이 silent 붕괴 — 타이틀 거짓말 신뢰 결함) **당일 수정**(`6a86d53`, union 의미론). 자문 4종(reviewer 0C/zod 0건/ai-orchestrator 문구 반영/crypto-trader) + 신규 `[10-105]`(로그 스케일·레퍼런스 라인·styleAxes 트리거).
  - ~~**4b — `[10-102]`(a) 크로스 metric 스크리너 + `[10-91]`/`[10-78]` 스키마 파생 강제**~~ ✅ **완결 (2026-07-12 당일 — 사이클 4 전체 완결 + 🎉 Stage 4 완료 선언. 단일 진실 = `task-record/M2-cycle4b-cross-screener.md` §6/§6b)**: 통합 `futures_indicators`(공유 const + "통합≡5 family union" 등치 불변식, table 공유=채널 0 증가) + dynamicColumns(**컬럼 = AI filters/sort 참조 파생, 큐레이션 0** — 사용자 재강조 → CLAUDE.md 명문화) + `[10-79]` 가상화 sticky 헤더 + superRefine 블록(3)(acceptsShapes×table 파생 스코프 강제, false-positive 0, kline 자연 면제). **라이브 G2-b 전 게이트 PASS**: "low LSR × rising OI" 크로스 필터 실증(구 5종 불가 조합) + 가상화 헤더 + 오유입 0 + 스코프 1차 완비 emit. deferred 회수 5건([10-102a]·[10-91]·[10-78]·[10-79]·[10-104]잔여) + 신설 [10-106]~[10-108].
- **▶ 다음 확정 (2026-07-13 계획 세션, 사용자 확정)**: ① ~~**`[10-100]` deferred 대장 대청소**~~ ✅ **완료 (2026-07-13 당일)** — 묘비 86건+인트로 묘비+이력 → `docs/deferred-archive.md` 신설 이관, 본 대장 332KB→219KB(열린 항목 209건), 헤딩 300/300 보존 불변식 PASS. 단일 진실 = `task-record/M2-[10-100]-deferred-cleanup.md` → ② **사이클 5 = Stage 1b (BigValue/Detail 일반화)** — 마지막 데이터-잠금 카드 2종(`ticker-card`→BigValue[scalar] / `indicator-card`→Detail[record]) 모양-제네릭 수렴 + `[10-74]`(descriptor 이중화) 자연 흡수. **★ 격자 완성 게이트 판정 (roadmap-mgr 2026-07-13): "격자 완성 = Stage 1b 까지"** — 완성 기준은 "데이터-잠금 안티패턴 부채 0"(히트맵/게이지는 완성이 아닌 *확장* — form 축은 무한이라 게이트 부적합) → **Stage 1b 완료 시 타 거래소·뉴스·ff#3 개방 재논의**(OKX 는 `[8-27]` #5/#6 구조 빚 + `[10-107]` 파일 분할 선행). ③ ~~**`[10-99]` UTC 표기 소사이클**~~ ✅ **완료 (2026-07-13 당일)** — 전 앱 절대 시각 UTC 통일(`formatEventTime`/`formatChartTime` timeZone + uPlot x축 `tzDate` + "UTC" 라벨 표시 지점 1회 소유) + 정책 명문화 `canonical-metrics.md §4.4` + 첫 결정 핀 테스트. 단일 진실 = `task-record/M2-[10-99]-utc-display.md`. 이후 후보 = `[10-84]` 청산 집계(reshape 첫 실증) → 히트맵 form → `[10-102]`(b) 크로스 테이블(future.md §5 derived datasource — 테마급).
- **사이클 5 — Stage 1b (BigValue/Detail 일반화) ✅ 완결 (2026-07-14 당일 완주) = 🎉 격자 완성 선언** (단일 진실 = `task-record/M2-cycle5-stage1b.md` — 완성 기준 4개 전부 충족: 데이터-잠금 부채 0[kline=의도된 예외 명시]·shape 전 form 커버·[10-74] 회수·G2 PASS. **▶ 타 거래소·뉴스·ff#3 개방 재논의 트리거 활성** — 다음 계획 세션에서 사용자 결정): 착수 세션 plan mode 승인 + 사용자 결정 3건 확정 — ① **kline-chart-card = 의도된 예외**(PRD §5 TradingView 정책 근거 특화 렌더러 — 격자 완성 선언 가능) ② **저장뷰 전체 삭제**(Stage 1 선례, 삭제 전 내용 보고) ③ **★ scalar shape = record 로 흡수**(BigValue 도 record 소비 — "큰 숫자"는 모양이 아니라 role=primary 표현 강조. 본 블록 ②의 "BigValue[scalar]" 표기는 record 로 정정). 5-step: [x] **Step 1 (2026-07-14)** — 통합 record descriptor 계약(`recordDescriptors.ts` role 축 primary/badge/secondary/detail, form 별 맵 금지=[10-74] 3중 회귀 방지) + pack 7벌(티커 공유 1벌 + 지표 5벌 verbatim 이관) + tone 헬퍼 `marketSemantics.ts` 분리(삭제 예정 파일에서 공유 시맨틱 탈출) + 불변식 13 테스트(★"같은 물리 table union" 기준 정밀화 — basis pack Mark/Index 적발 계기 + 옛↔새 기계적 등가 1). reviewer 0C(W1 COINM volume 단위 분기·W3 등가 테스트 즉시 반영). 화면 무변경, web 478 green / [x] **Step 2 (2026-07-14)** — 공유 엔진 훅(`useSingleRecord` = 옛 두 카드 동형 중복 ~120줄×2 이식 + `useReconnectIndicator` 옵션 C) + **BigValueCard**(role 소비: primary huge+flash / badge / secondary / detail skip — "OI as big number" 단위 테스트 실증). reviewer 0C(W1 과도기 동결 규율·W2 Step 4 양방향 등치 승계·S2 유령 flash 즉시 수정) / [x] **Step 3 (2026-07-14)** — **DetailCard**(전 role 필드 선언 순서 렌더 — premium_index 라벨 순서 = 옛 IndicatorCard 동일 박제, 티커 Detail 전 10 필드 = 신규 조합). web 488 green / [x] **Step 4 (2026-07-14)** — 원자적 스왑: big-value-card/detail-card 등록(record 7종 union, superRefine 스키마 0줄 자동 파생) + 옛 2 카드·indicatorDescriptors 삭제 + scalar 판정 박제(shapeKind) + 프롬프트 예시 교체·**detail 예시 신규**(혼동 쌍 앵커링 균형, ai-orchestrator 자문) + fixture sweep 16파일(reject 픽스처 의미 보존 교체) + **양방향 등치·렌더 매트릭스 14쌍 핀** + 저장뷰 실측 0행(삭제 불요). 자문 2종 0C(zod 전항목 PASS / reviewer W1 = ★AI-facing 주입 문자열 잔존 적발→즉시 수정, 메모리 보강). shared 104+web 483 green / [x] **Step 5 (2026-07-14)** — 라이브 G2 전 게이트 PASS: site=DB + ★직교 교차 실증("OI as a big number"→BigValue×OI, role 소비 정확) + COINM Detail contracts 단위 + AI 분기 + 콘솔 0 + crypto-trader 자문(★0%=중립 지지, UX 관찰 5건=`[10-111]`). **G2 적발 결함 2건 당일 근본 수정·배포·검증**: ① 차트 커서 stale rect(1차 배율 보정의 재발 — cursor.move 를 원본 이벤트+fresh rect 직접 환산으로, 전 캔버스 일반 적용) ② **COINM 티커 full 승격**(mini 잔존이 24h 필드 매초 null 클로버링 + q→quote_volume 오적재 — 스모크가 **dstream/fstream ticker 도 UM+CM 병합 실측 = `[10-14]` 2차 적중** → st 2단 가드, DB 실측 정합·산식 일치). 부수 = 워커 반쪽 shutdown 실사고(`[10-110]` 신설, 다음 워커 배포 전 회수 권장). → **격자 완성 선언 ✅**.
...
- **🏁 M2 종료 선언 (2026-07-15 계획 세션)**: 확장 루프 12회전(테마 A/B/C + [10-33] + 경로 A + ff#1·#2 + 사이클 1~5) + **격자 완성**(2026-07-14)으로 M2 마감. M2-plan 은 `task-record/M2-plan.md` 로 이동, usage-feedback 은 마감(이후 기록처 = M3-plan §6). **▶ 이후 = 아래 §M3.**

---

## M3 — "Binance 우주 완성" (2026-07-15 신설 — 신규 공급자 추가 전 마지막 다듬기)

> **단일 진실 = `docs/M3-plan.md`** (살아있는 계획서 + 실사용 백로그). 진행 방식은 M2 와 동일한 확장 루프(사이클 단위, 7단계 표준 절차) — 마일스톤 이름만 M3 로 전환.

- **정의**: 신규 데이터 **공급자(provider)** 추가 전, **Binance 데이터만으로** 모든 데이터 × 모든 형태 × **모든 인터랙션** + 고도화 + UIUX 다듬기 완성. 경계 = 공급자 기준(Binance 내부 datasource 추가는 IN — 알파/체결·호가/추가 metric, 새 공급자 CMC·뉴스·OKX·온체인은 OUT = M4 벽. `[8-27]` 리팩터도 M4 벽 앞에서).
- **트랙 5개**: ① 인터랙션 wire(PRD §4 spawn/drill-down 실동작 — 마지막 미구현 조각) ② 표현력 잔여(`[10-84]` 청산 집계 → `[10-102]`(b) 크로스 테이블 → 히트맵 form → `[10-93]`/`[10-105]`/`[10-88]`…) ③ Binance 내부 소스 확장(ff#3 체결·호가 / 알파[API 조사 선행] / 추가 metric) ④ UIUX 다듬기(`[10-111]`/`[10-109]`/`[10-83]`/`[10-21]`/`[10-67]`…) ⑤ 운영·부채 동반(`[10-110]` 워커 배포 Step 0 강제 / `[10-86]` ticker coalescing).
- **첫 사이클 (사용자 확정 2026-07-15) = 인터랙션 wire + UX 즉결 웜업** — **✅ M3-step1 완결 (2026-07-16 당일 완주)**: Spawn 실동작("AI 사전 선언" 계약 + 클릭 조립 0초) + 웜업 4건(`[10-111]`①②④ + `[10-109]`① — ② 는 사용자 미채택) + 라이브 G2 전 시나리오 PASS + ★AI 자율 actions 선언 실증(매핑 규칙 0). drill-down 등 잔여 인터랙션은 `[4-13]`. 단일 진실 `task-record/M3-step1-interaction-wire.md`.
- **✅ M3-step2 완결 (2026-07-17 당일 완주) = 인터랙션 완성 2탄**: `[10-113]` 뷰포트 빈자리 배치+만차 Show/Undo 토스트(자동 팬 기각) + `[10-115]` 재클릭 체인 **깊이 1 명시 중첩**(재귀는 `$refStrategy:"none"` 이 `{}` 로 뭉개 기각) + AI 타겟 다양성 + hover 힌트("View detail ↗"). ★프로덕션 G2 에서 `[10-114]` 첫 실측 발현(AI marketType 통로 누락 선언 → 전 클릭 실패, 사용자 적발) → **엔진 암묵 스코프 선보충(registry 파생)+issue 보충+프롬프트 강화**로 당일 해소·저장 선언 소급 구제. 검증 = 로컬 Playwright 결정 3/3(정규 승격 `m3.2-spawn-viewport.spec.ts`) + Anthropic 실 1콜(AI 체인 자율 emit) + 프로덕션 체인 완주("표→상세→BINANCE:LUMIAUSDT 1D"). 회수 `[10-113]`·`[10-114]`·`[10-115]` / linked_selection 사용자 기각(`[4-13]` 기록). 단일 진실 `task-record/M3-step2-interaction-2.md`.
- **✅ M3-step3a 완결 (2026-07-19 당일 완주) = `[10-84]` 청산 events→series 집계 Phase 1**: 청산을 시간버킷 집계(series)로 서빙해 chart form 에 **코드 0줄로 유입** — Form↔Data 직교 실증 3호. 집계는 **DB 함수(RPC)** — 클라 집계는 `FETCH_HARD_CAP`(3000행)에 걸려 전 시장 24h(24,692행)이 **무증상 절단**되는 사이트=DB 위반이라 기각(DB 집계 실측 85:1 / 37ms / Seq Scan 0). registry 3축 가산(`fetchKind`·`rpcSpec`·`optionalScopeFields`, 전부 default 보유 = 기존 15 datasource 무변경) + superRefine (3) 일반화 + `[10-81]` 현재시각 주입 회수. ★ **`total_notional` 한 컬럼이 3형태를 덮는다**(RPC 가 side 를 pushdown — 롱만/숏만/합계. 다이버징만 Phase 2). **라이브 G2 10게이트 PASS**: ★파생 parity(RPC ≡ 독립 SQL **센트 단위 일치** — Binance 가 청산 집계 화면을 제공하지 않아 site=DB 대조 표면이 없는 데이터의 **대체 검증으로 신설**) / notional 절벽(07-05 이전 `total=NULL`→**gap**, 0막대 위장 없음) / AI 자율 4형태·4버킷 / 진행 중 버킷 제외 / 회귀 0. G2 적발 결함 1건(**계층 능력 도달 불가** — 훅이 빈 symbols 를 아래 분기 앞에서 idle 로 끊음) + code-reviewer Critical 2(`exchange` 경로별 내성 비대칭 / 방어 신호 미배선) 당일 반영. **부수: 🔴 청산 마켓 오분류 1,232행 규명·차단·정리**(원인 = `!forceOrder@arr` 에 `st` 판별자 **부재** → fail-open 폴백이 신규 상장 심볼 통과. 워커 근본 수정 = `[10-117]`). 신규 `[10-117]`~`[10-121]`. 단일 진실 `task-record/M3-step3a-liquidation-series.md`.
- **✅ M3-step3b 완결 (2026-07-20~21) = `[10-121]` 청산 롱/숏 다이버징 한 차트 (Phase 2)**: Step 0 강제 분할(`[10-98]`/`[10-120]`① — chartFormat→`chart/` 6모듈 barrel + ChartCard→훅 2+오버레이, 순수 이동 510 green) → `style.breakdown` AI 계약 + descriptor `breakdown` 성분 선언 + `resolvePlotSpecs` 진리표(side 필터→단일 / 오버레이→total 폴백 / AI 명시 ?? 도메인 기본) → **다이버징 렌더**(숏 표시층 invert·y축 공유 단일 스케일·방향 2색·midline 0 파생·축 abs·툴팁 원값+"N events" getter) + `[10-120]`② 시간창 배선(fromIso/toIso — rpc p_from/p_to + table gte/lte, history 6종 선재 갭 동시 해소) + `[10-83]` 사용자 결정 3건(농도 log1p 채택 등). reviewer C0 + W2 반영(무음 절단 "window trimmed" 고지 / 오버레이 폴백 스키마 광고). **라이브 G2 PASS**(다이버징 기본 실증 + total/side AI 자율 분기 + 시간창 12:00~20:00 실반영 + 툴팁 ≡ 독립 SQL 센트 일치 + OI 회귀 0). ★배포 사고 = **Vercel Git 자동배포 1일 불발**(빌드 실패 아닌 트리거 부재 — CLI `vercel deploy --prod` 해소, 익일 자동 복귀) / ★**AI emit miss 3건 실측** → 서술 완결 처방(side 통로·**도메인 기본값 사실 고지**)으로 2건 해소, 오버레이 `symbol in` 누락은 `[10-122]` 관찰. 회수 `[10-83]`·`[10-98]`·`[10-121]`·`[10-120]`①②⑥(a~c). **▶ 다음(예약) = 린 워커 위생 사이클**(`[10-117]`+`[10-110]` Step 0+`[10-118]` 차선책, `[10-86]` 제외 — 사용자 확정). 단일 진실 `task-record/M3-step3b-chart-multicolumn.md`.
- **✅ M3-step4 완결 (2026-07-22 당일 완주) = 린 워커 위생**: `[10-110]` shutdown 풀 견고화(runGracefulShutdown 추출 — allSettled 독립 정지 + flush 보장 + watchdog 25s + exit 0/1/64. ★근본 원인 규명 = **TierPoller.stop 무한 대기** → 5s 유예 = **재시작 7분 수동 좀비 → 11초 자동 완주** 라이브 실측) + `[10-117]` 청산 마켓 판별 2단 가드(★라이브 캡처 8/8이 "st 부재" 진단 재정정 — `st` 는 **`o` 객체 안**에 실림[문서 예시는 최상위 = 문서↔와이어 위치 괴리], `o.st` 권위 판별 + fail-closed inclusion 폴백. crypto-domain W3 교차판정 = 저장 경로 안전) + `[10-118]` 차선책(refresh 24h→1h) + **🆕 신규 상장 자기 재시작**(diff 감지 → graceful exit 64 → systemd 재기동 = per-symbol 구독 재구성. 사용자 조건 "수요일 19시 상장 → 데이터 부재 불가" 충족 — 전 데이터 축 ≤1h. 루프 가드 2중 = 1h 구조 간격 + 대량 추가>20 억제[reviewer W1]). **라이브 G2 ⓐⓑⓓ PASS**(ZZTESTUSDT 주입→1h 틱 정확 감지→재시작→부팅 스냅샷 반영). G2 적발 1건(성공 exit 64 의 거짓 "실패" 로그) 당일 수정. 회수 `[10-110]`/`[10-117]`/`[10-118]`차선책분. 단일 진실 `task-record/M3-step4-lean-worker-hygiene.md`.
- **명시 보류**: 세션 컨텍스트(future §4)·혼합 응답(future §3) — 수요 재실증 시 재논의 / OKX = 미정(베타~Launch 사이 재논의, Launch 전 필수 `[6-1]`).
- **M3 이후 스케치 (가칭, `M3-plan.md §3`)**: M4 = 외부 데이터소스 개방(CMC·뉴스, `[8-27]`#1~#4 선결) → M5 = 베타 준비(M1.7 Closed Beta Ops 활성화 + 100명 스케일 `[10-86]`/`[10-54]`~`[10-56]`/`[3.5-3]` + 보안 종합 감사 + 전체 코드 정리) → 베타테스터 모집 → Launch §L.
