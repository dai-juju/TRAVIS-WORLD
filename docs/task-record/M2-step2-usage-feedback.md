...
> **▶ 현재 (2026-07-09 #2)**: 확장 루프 **7회전 사이클 1 ✅ 완결(G3 PASS = `[10-77]` 묘비)** + **8회전 사이클 2(GenericChart) 🔄 Step 1~6 코드+데이터 ✅ — 잔여 = Step 6 라이브 G2 만**. Step 5 라이브 G2 전 게이트 PASS(AI 자율 분기 5/5 + ★"compare" 직교 실증 3호 + hotfix 4연쇄) + UIUX 4결정 + **Step 6 펀딩(§4h)**: 별도 이벤트 테이블 `history_futures_funding`(도메인 자문이 "기존 컬럼 채움" 기각 — interval 버킷 복제=가짜 포인트) + funding rate-limit 버킷(Plan 검증 결함 적발) + **backfill 완주**(USDM 196,600행/60일 + COINM 3,580행, 라이브 smoke 가 페이지 절단 유실 적발→수정) + chart-card bars/stepped 어휘(펀딩 전용 카드 아님). reviewer 2회 0C. 신규 `[10-94]`~`[10-96]`. 단일 진실 `M2-cycle2-genericchart.md §4h`. **다음 세션 = Step 6 별도 라이브 G2(§4h 끝 게이트 7종) → 통과 시 사이클 2 완결**. 실사용 병렬 계속(`[10-83]` advisory 관찰).
