...
- **Step 3 — 순차 롤아웃 + 검증** 🟡 **진행 중 (2026-06-04)** (단일 진실 `docs/task-record/M1.9-step3-rollout.md`):
  ...
  - **후속(벽시계·다음 세션 의존)**: ⓐ/ⓒ/[8-33] + 즉효 fix **라이브 재배포 검증**(staggered 로그+basis 호출 감소+-4104 소멸+shutdown inactive) → USDM 24~48h site=DB(G2, 9 interval+ban 0) → `FORWARD_FILL_COINM=1` COINM ON → ~1달 누적 → **잔여 ⓑ AbortSignal/ⓓ circuit breaker**(shutdown 품질, 병행 가능) → G1~G5 게이트 + `M1.9-complete.md`.
