...
3. **▶ (지금) 사이클 1 = Realtime throttle `[10-77]` + favicon `[10-82]`** — ★ **착수 확정 (사용자 2026-07-07)**: Supabase Realtime "Message Count Exceeded" 경고의 grace 가 7/22(2주 앞)라 선행. markPrice 는 **DB 쓰기만** 30~60초 coalescing(화면 실시간성 = 경로 A WS 담당이라 체감 0, DB 사본 = AI 쿼리·site=DB 용). `backend-infra-specialist` 측정 선결(현재 upsert 빈도·StreamCoalescer 현황·markPrice 만 선별 가능 여부·mixed-batch 불변 유지). **배포 후 검증 게이트 = Supabase MCP**(`updated_at` 간격 실측 + advisors + logs) **+ Dashboard usage 추세 후속 관측**(Realtime 메시지 카운트는 MCP 미노출 가능 → 사용자 협업, COINM 안정성 체크 선례).
