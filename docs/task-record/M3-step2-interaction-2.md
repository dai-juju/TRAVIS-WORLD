...
**완료일**: 2026-07-17 (당일 완주 — 프로덕션 결함 적발·수정·재검증 포함)
**예상 소요**: 8~12h
...
| 5 | 라이브 G2 종합 + docs/deferred 정산 | ✅ 로컬 결정 3/3 + 사용자 체크리스트(①② PASS, ③ 결함 적발→당일 수정) + 프로덕션 재검증 + 정산 완료 |

### 사이클 마감 정산 (2026-07-17)

- **회수**: `[10-113]`·`[10-114]`·`[10-115]` — deferred 제거 + archive 전문 이관 (열린 항목 212→209).
- **사용자 결정**: linked_selection **기각**("다양한 코인을 쌓아 볼 수도, 하나만 볼 수도 있어 굳이" — `[4-13]` 에 의견 기록, 수요 실증 시 재상정) / Playwright 회귀 테스트 **정규 승격**(`tests/e2e/m3.2-spawn-viewport.spec.ts`, 로컬 수동 실행).
- **다음(M3-step3) 후보**: `[10-84]` 청산 집계(기존 1순위) / ff#3 / 히트맵 / `[10-102]`(b) — 다음 세션 plan mode 에서 선정.
