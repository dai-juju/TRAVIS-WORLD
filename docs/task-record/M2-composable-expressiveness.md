...
> **상태**: ✅ **Stage 1 완료 (Step 1~5, 2026-06-30)** — `table-card` 가 옛 카드 2종(`coin-list-card`/`indicator-list-card`)을 **라이브 대체**. 게이트 registry 권한 일원화 · alias 대신 저장뷰 삭제 · all-view 일관화(`defaultLimit` 필드 제거). **라이브 G2 PASS**: top gainers/all spot(449 가상스크롤)/**top OI=옛 F3 "realtime error" 해소(site=DB 자릿수 일치)**/funding + 단일/다중 의도분리(zod S2 해소) + COINM 단위 + 콘솔 0. 커밋 `2a964bd`(Step 3+4)·`f1221ae`(docs)·all-view 수정(이 세션 커밋). **사이클 1(`[10-77]`+`[10-82]`) ✅ (2026-07-07, G3 usage 관측만 잔여 — `M2-[10-77]-realtime-throttle.md`). ▶ 사이클 2 GenericChart 🔄 Step 1~4 ✅ (2026-07-08 하루 완주)** — 계획 승인(6-step, 사용자 확정 4건: 착수/펀딩 0행 실측→후반 Step 6/오버레이=(a) 다중 심볼만·(b) 이중축=Stage 4 펜싱/**uPlot**) + Step 1 Shape 계약(2층 게이트) + Step 2 `useDataServiceSeries`(첫 주기 pull) + Step 3 history datasource 6종+chartDescriptors + Step 4 GenericChart form(미등록 격리). ⚠️ **Step 3·4 = 로컬 커밋만(`09cb300`·`d16863c`), push 는 Step 5 와 원자적 일괄**(사용자 결정 — over-advertisement 퇴행 창 방지). **단일 진실 = `M2-cycle2-genericchart.md`** (§11 항목 4 의 실행체 — Step 5 TODO 포함). **다음 세션 = Step 5 (등록 + 일괄 push + 라이브 G2).**
...
3. ~~**사이클 1 = Realtime throttle `[10-77]` + favicon `[10-82]`**~~ — ✅ **완료 (2026-07-07 당일 완주, G3 관측만 잔여)**: `MarkPriceWriteCoalescer` 60초(사용자 확정) 구현 + code-reviewer 0C(W1 shutdown 경합 즉시 수정 = 메모리 `feedback_async_coalescer_flush_guard_shutdown`) + Hetzner 배포 → **G1·G2 PASS**(`updated_at` 60초 단일 클러스터 695행 MCP 실측 + advisors 신규 0 + 배포 후 deadlock 0). favicon `a62183c` = `[10-82]` 묘비. 부수 발견 `[10-86]`(ticker 테이블이 더 큰 churn 후보 — G3 결과로 판단). **잔여 = G3**(Dashboard usage 추세 며칠 + crypto-trader advisory + favicon 404 소멸 육안). 단일 진실 = `M2-[10-77]-realtime-throttle.md`.
4. **(🔄 Step 1~4 ✅ 2026-07-08 — 단일 진실 = `M2-cycle2-genericchart.md`, 남은 것 = Step 5[등록+일괄 push+G2]·Step 6[펀딩] / Step 3·4 는 로컬 커밋만 — push 는 Step 5 와 원자적 / 펀딩 적재 = 후반 Step 6[0행 실측 확정] / 오버레이 = (a) 다중 심볼만[(b) 이중축 = Stage 4 펜싱] / 차트 엔진 = uPlot 확정) 사이클 2 = GenericChart** — Stage 2 `series` 데이터 레이어(`useDataServiceSeries` 범용 history fetch + ★ **shape 자동판정 정식화**: datasource 의 서빙-가능-shape 선언 × form 의 `acceptsShapes` 선언 **교집합 게이트** — "새 데이터 등록 → 전 form 과의 가능/불가 자동 판정", 사용자 원칙 2026-07-07: 하드코딩 매핑 절대 금지·AI 가 의도로 form·data 자율 선택, 게이트는 무의미 조합만 거르는 호환성 선언) + Stage 3 차트 form(chart-card, 라이브러리는 UHD620 저사양 기준 nextjs-frontend 자문). **범위 = series 공급 가능한 전부 (사용자 확정 2026-07-07)**: `_history` 6 metric(OI/LSR 3종/taker/basis) + **펀딩 히스토리**(★ 선결 확인 — backfill/forward-fill "6 metric" 목록에 펀딩 없음 → DB 채움률 실측, 비어 있으면 collector-history 7번째 task 가산 + 과거 backfill, 결제주기 이산성[8h/4h/1h]은 crypto-domain canonical 자문). `refreshInterval` 첫 실사용(ff#2 Step 4 에서 "진짜 주기-pull datasource 등장 때"로 연기해 둔 것). 청산 events→series 시간버킷 집계 배관 = `[10-84]` 후속(배관 생기면 chart form 0줄로 청산 유입). 가격 캔들 = TradingView 임베드 정책 유지.
