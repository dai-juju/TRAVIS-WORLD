...
- **★ 실측 보강 (2026-07-15, 현 주기 usage 점검 — 사용자 Dashboard 실측)**: 현 주기(6/21~7/21) 2,520,573/5M(50%, overage 0)로 통과 확실시. 단 일별 그래프가 **세션 구동형 패턴** 실증 — 무세션일(7/11·7/13) ≈ 0 vs 사이클 2 라이브 G2 장시간 세션일(7/10) 단일 **~760K 스파이크**(주기 총량의 30%). markPrice 코얼레서(7/7)가 바탕 사용량은 잡았고, 잔여 지배 원천 = **활성 세션 중 ticker 테이블 realtime 구독**(table-card "top gainers" 류)일 가능성 높음 — 초당 2,160 × 6분 ≈ 780K 로 스파이크 크기 정합. 테이블별 기여 실측은 여전히 선결이나, **다음 워커 작업 시 ticker write coalescing(예: 5~10초) 동반 회수 검토로 승격**(freshness 트레이드오프는 crypto-trader 자문).
- **해결 힌트**: `[10-77]` 의 `MarkPriceWriteCoalescer` 패턴 재사용(단 ticker 는 partial 아닌 full upsert 라 mixed-batch 고려 상이). 선결 = ~~`[10-77]` G3 관측 결과~~(✅ 07-15 완료) + Realtime 메시지의 테이블별 기여 실측. **블록킹**: No. **카테고리**: 🔵 Launch Readiness → **다음 워커 작업 동반 후보** (07-15 승격). **출처**: `M2-[10-77]-realtime-throttle.md §2-8` + 2026-07-15 usage 실측.
...
- **② tool input_schema 비대 관찰** (깊이 1 중첩 대가): 11,240 → 18,734 bytes (+66%, CardDataBinding 4곳 인라인) = 콜당 입력 +~2K 토큰(Haiku ~$0.002, 현재 수용). **트리거**: 스키마가 또 크게 늘 때(3-hop·신규 액션 타입) CardDataBinding `.describe()` 다이어트 검토.
- **③ 만차 3중 폴백 대각 오프셋의 소스 겹침** (reviewer S2): 캔버스 완전 만차 + 화면 밖 cascade 12회 소진의 극단 케이스에서 `{src+40,+40}` 이 소스 위에 겹침(드래그로 정돈 가능) — 현행 수용, 실사용 발생 시 재검토.
- **④ 저사양 Low-tier 팬 duration:0 폴백** (nextjs-specialist 권고): 토스트 "Show" 의 300ms 팬이 UHD620 에서 버벅이면 즉시 점프 폴백 — 실사용 체감 시 소회수.
