...
| **8.3b-2** | Hetzner deploy + journal 5분 관찰 + 시뮬레이션 출력 6 항목 검증 + 사용자 D20/D21/D22 결정 | — | journal 에서 5분당 1회 `[historyBackfillTask:dry-run]` 시뮬레이션 출력 + 다른 4 task IP quota 위반 0건 | ~30분 |
| **8.3c** (선택) | 실 backfill 진입 — BinanceUsdmAdapter history fetcher 5종 + normalize + loop + ~20M row INSERT | — | history_futures_indicator rows > 20M (실 backfill 완료) + IP quota 위반 0건 + NRestarts 0회 | 사용자 결정 후 별도 산정 |
