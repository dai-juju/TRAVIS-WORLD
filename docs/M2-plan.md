...
> **현재 위치 (2026-06-04)**: **M1.9 진행 중** — **Step 0 ✅**(transient/auth/quota 3분류 `[3-68]`) + **Step 1 ✅**(collector worker 인프라 추출 `[8-20]`) + **Step 2 ✅ 코드 완성**(forward-fill USDM+COINM 실 구현, `[8-26]`/`[8-3]` 회수) + **Step 3 🔄 라이브 가동 중**(2번째 Hetzner 서버 49.13.138.121 별도 IP USDM 배포 → history **05-31 정지→06-04 현재 복구 실증** + freshness 인덱스 25초→5.9ms + 즉효 fix 3종). **다음 = (a) 근본 fix `[8-31]`**(프로세스 전역 shared rate limiter + AbortSignal 협조적 취소 — basis ban·shutdown SIGKILL 구조 해소, COINM 롤아웃 전 필수) **→ (b) USDM 24~48h site=DB(G2) → (c) COINM 롤아웃 → (d) G1~G5 종단 게이트 + M1.9-complete.md → 그 후 본 §Step 2 실사용/베타**. ⚠️ `[8-26]` 정지 문제는 코드 차원 해소(가동 검증 진행 중). 단일 진실: `docs/ROADMAP.md §M1.9` + `docs/task-record/M1.9-step3-rollout.md`.
> **세션 재개 단일 진실 원천 (M1.9 Step 3 후속 착수용)**: **`docs/task-record/M1.9-step3-rollout.md`** (§"후속 세션 인계" = ① 근본 fix `[8-31]` 6항목 ② COINM 롤아웃 전 합산 req/min 재확인 ③ 24~48h site=DB ④ 종단 게이트 **필독**) + **`docs/ROADMAP.md §M1.9`** + **`docs/deferred-task.md [8-31]`**(근본 fix 상세) ← `/clear` 후 가장 먼저 읽을 파일. (Step 2 이력 `M1.9-step2-forward-fill.md`, M1.8.5 이력 `M1.8.5-complete.md`.)
