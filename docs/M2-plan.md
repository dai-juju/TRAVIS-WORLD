...
> **★ 2026-06-09 갱신 (테마 A Step 0·1·2 완료)**: 테마 A 6-step 분해 완료. **Step 0(F3 즉시 안전망)·Step 1(datasource id≠테이블 분리, `[8-27]`#1)·Step 2(IndicatorCard + `[10-7]` 회수) ✅ 완료**. Step 2 = 단일 심볼 지표 카드(funding/basis/OI/LSR/taker datasource 별 적응 렌더, generic 1종 + descriptor), premium_index drift 재정합(predicted/realized 분리) + basis datasource 신설 + watchColumns fan-out dirty check. 6패키지 type-check + web 156/shared 30 test green, code-reviewer Critical 0. **단일 진실 = `docs/task-record/M2-themeA-card-expressiveness.md §4`**. **▶ /clear 후 첫 작업 = 테마 A Step 3 (IndicatorListCard — 멀티 심볼 랭킹 리스트).** ★ 라이브 site=DB G2(카드 육안)는 Vercel 배포 후. 신규 deferred `[10-9]`(표시 라벨)·`[10-10]`(enum/한국어 cleanup).
>
