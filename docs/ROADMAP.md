...
| **8.4** 🟡 | SPOT stale row cleanup — non-TRADING DELETE + worker upsert TRADING 필터. **8.4-a 코드 fix ✅ 2026-05-26** (ticker24hrBatchTask 에 tradingSymbolsByMarket allowlist 적용, tickerWsHandler 패턴 미러링 — `docs/task-record/M1.8-step4-spot-cleanup.md`). **8.4-b DB DELETE + 8.4-c Hetzner deploy + 검증 🟡 진행 예정** (사용자 Supabase Studio SQL Editor + SSH). | `[3-53]` (회수 예정), 진단 보고: SPOT 60% NULL 의 진짜 원인 = ticker24hrBatchTask 의 partial upsert 가 BREAK 심볼 INSERT (tickerWsHandler 는 이미 TRADING 필터 적용) | `now_spot_ticker` NULL 비율 < 5% + 24h 후 stale 재발 0건 + ticker24hrBatchTask 가 TRADING 만 upsert | 1~2h |
