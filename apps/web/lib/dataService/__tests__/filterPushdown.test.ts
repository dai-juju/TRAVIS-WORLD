...
//   "=" string → eq / "in" → inFilters / 그 외("!=", ">", number "=") 는 제외
...
describe("splitServerFilters", () => {
  ...
  it('비교 연산자(">")·"!=" 및 number "=" 는 pushdown 제외 (클라이언트 전용)', () => {
    const out = splitServerFilters([
  ...
  it("filters undefined / 빈 배열 → 빈 결과 (graceful)", () => {
    expect(splitServerFilters(undefined)).toEqual({ eq: [], inFilters: [] });
    expect(splitServerFilters([])).toEqual({ eq: [], inFilters: [] });
  });
});
