...
> **이관 이력**: 2026-07-13 최초 이관 86건 (+ §1/§2 인트로 묘비 + 구 집계표/🚦 이력 부록) / 동일자 `[10-99]` 회수 이관 (신규 규칙 1호).
...
## ✅ M3-step4 회수 (2026-07-22, 린 워커 위생 — 단일 진실 `task-record/M3-step4-lean-worker-hygiene.md`, 커밋 f1e9808→8c7f3b2→893729b)

### [10-110] 워커 재시작 절차 견고화 — tsx 부모 SIGTERM 반쪽 shutdown (WS 공백 ~7분 실사고) ✅
- **원문**: sudo-free 재시작 관례(MainPID kill)가 실패 — SIGTERM 이 자식 앱의 graceful shutdown 훅을 태워 전 WS relay 를 닫았는데 자식 프로세스가 종료되지 않음 → systemd 재시작 불발 = ~7분 수집 공백(반쪽 좀비, 2026-07-14). 해결 힌트 = hang 경로 조사 + 종료 타임아웃. 출처 `M2-cycle5-stage1b.md §8c`.
- **회수**: `runGracefulShutdown` 추출(allSettled 독립 정지 + flush 순차·독립 보장 + watchdog 25s + exit code 0/1/64 분리 + hang 컴포넌트 이름 지목) + ★**근본 원인 규명 = `TierPoller.stop()` 의 in-flight execute 무한 대기**(perSymbolTask 수 분 사이클) → `stop(graceMs=5s)` 유예 상한. 라이브 G2: SIGTERM 재시작 **총 ~11초 exit 0** 완주(종전 7분 수동 개입). 회귀 테스트 7종 신설.

### [10-117] 🔴 청산 마켓 판별 fail-open 근본 수정 ✅
- **원문**: COINM 라벨 오염 1,232행(2026-07-09~17) — ① `st` 분기가 죽은 코드("페이로드에 없음"으로 진단) ② 폴백이 fail-open("상대 allowlist 확인 시만 drop")이라 신규 상장 심볼 통과. 수정안은 심볼 구조 불변식(`_` 판정) 제안. DB 트리거로 완화 중. 출처 `M3-step3a §착수 중 발견`.
- **회수**: ★미확정 검증(라이브 1프레임)이 **진단을 재정정** — `st` 는 부재가 아니라 **`o` 객체 안에** 실림(8/8 프레임, 문서 예시는 최상위 = 문서↔와이어 괴리. 구 코드의 최상위 읽기가 죽은 분기의 실체). → 구조 불변식 대신 **2단 가드**: ① `o.st` 권위 판별(신규 상장 0초 면역, dated 정확) ② st 부재 폴백 = fail-closed inclusion("경로 B 이력 보존" 정책 폐지 — 청산은 TRADING 심볼에서만 발생, crypto-domain W3 교차 판정으로 저장 경로 안전 확인). dedup 거짓 주석 정정 + drop 관측 로그(심볼당 1회). DB 트리거는 2중 안전망 존치. canonical-metrics·메모리·step3a 서술 재정정 전파.

### [10-118] (부분) 신규 상장 청산 무음 손실 — 차선책 + 자기 재시작 ✅ (근본 동적 재구독은 🟢 잔존)
- **원문**: per-symbol chunked 구독이 부팅 스냅샷 고정이라 신규 상장 청산이 재부팅까지 통째 유실(SKHYUSDT 3일 0건 실측). 차선책 = refresh 24h→1h.
- **회수분**: `SYMBOL_REFRESH_INTERVAL_MS` 1h + **신규 상장 자기 재시작**(planSymbolRefresh diff 감지 → graceful exit 64 → systemd 재기동. 루프 가드 2중 = 구조적 1h 간격 + 대량 추가(>20) 억제[reviewer W1] + 빈 리스트 마켓 swap 생략). 라이브 G2: ZZTESTUSDT 주입 → 1h 틱 정확 감지 → 11초 재시작 → 부팅 스냅샷 usdm=720 반영 실측. "수요일 19시 상장 → 20시 전 전 데이터" 사용자 조건 충족.
