...
- **M1.9 — history 시계열 지속성(forward-fill) + COINM 확장** 🔄 **진행 중** (`docs/ROADMAP.md §M1.9`) — M1.8.5 가 채운 history 가 2026-05-31 정지(`[8-26]`)한 문제를 별도 Hetzner worker(별도 IP)로 forward-fill 24/7 가동해 해소 + COINM 확장. **Step 0 ✅** (2026-06-02, `transient_error` → auth/quota/transient 3분류, `[3-68]`) / **Step 1 ✅** (2026-06-02, collector worker 인프라 추출 — `packages/exchange-collectors` + `@travis/shared` TierPoller 승격 + `apps/collector-history` 골격, `[8-20]`). **Step 2 ✅** (2026-06-04, forward-fill 실 구현 USDM+COINM 코드 완성, `[8-26]`/`[8-3]` 회수) / **Step 3 🔄** (2026-06-04~05, 2번째 서버 별도 IP 라이브 가동 — USDM history 05-31→현재 복구 실증 + freshness 인덱스 25초→5.9ms + 즉효 fix 3종. **근본 fix `[8-31]` ⓐⓑⓒ 전부 회수 완료 (2026-06-05): ⓒ per-metric throttle + ⓐ /futures/data token-bucket(opt-in) + ⓑ AbortSignal 협조적 취소 + `[8-33]` 금속 basis 제외. 라이브 재배포 #1·#2 완료, 5m·1h lag 3분.** 잔여 ⓓ circuit breaker(차단 아님)·COINM 롤아웃·24~48h site=DB·종단 게이트는 후속). 단일 진실 `task-record/M1.9-step3-rollout.md`.
